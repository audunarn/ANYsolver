"""Bounded isolated G3c SHELL development tests; never formal qualification."""
...
REVIEW='docs/reference_cases/ge_beam3_g3c_matrix_shell_equation_review_v1.json'
REVIEW_SHA='bde2b65eea863399d3fc15929e99e40da4460cb641024503ecf6d2eea74c0062'
TEST='tests/test_ge_beam3_g3c_local_shell.py'
SMOKE=[TEST+'::test_independent_pose_actual_reference_rank_and_rigid_modes[square]',
       TEST+'::test_independent_pose_actual_reference_rank_and_rigid_modes[right]']
...
def authority():
    ...
    if r['decision']!='ACCEPTED_G3C_MATRIX_SHELL_EQUATIONS_LOCAL_IMPLEMENTATION_ONLY' or r['findings']:
        raise ValueError('shell equation authority missing')
    contract=(ROOT/'docs/reference_cases/ge_beam3_g3c_matrix_shell_contract_v1.json').read_bytes().replace(b'\r\n',b'\n')
    if sha256(contract).hexdigest()!=r['scope']['contract_sha256']: raise ValueError('changed shell contract')
...
def worker(out,lease_hash):
    ...
    if lease['lane'] not in ('smoke','full'): raise ValueError('unregistered shell lane')
...
def main():
    ...
    if sys.argv[1:] not in ([],['--smoke']) or os.name!='nt': raise ValueError('fixed Windows shell lane required')
    lane='smoke' if sys.argv[1:]==['--smoke'] else 'full'
    if not(sys.flags.isolated and sys.flags.no_site and sys.dont_write_bytecode):
        raise ValueError('launch with -I -S -B')
    authority()
    out=Path(tempfile.mkdtemp(prefix='anysolver-g3c-shell-development-'))
