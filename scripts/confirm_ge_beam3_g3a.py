"""Fresh, bounded, hash-bound G3A S15/native S18 confirmation; no general parity claim."""
import argparse
...
INVENTORY = ROOT / "docs/reference_cases/ge_beam3_g3a_confirmation_inventory.json"
TESTS = ["tests/test_ge_beam3_g3a_graph.py"]
ACCEPTED = "ACCEPTED_G3A_IMPLEMENTATION_REVIEW"
...
def git(*args):
    return subprocess.check_output(["git", "-c", "safe.directory="+ROOT.as_posix(), *args], cwd=ROOT, timeout=20).decode().strip()
...
def identity(candidate, *, clean=True):
    ...
    paths = sorted(p for p in git("ls-files").splitlines()
                   if p.startswith("src/") or p in TESTS or p in (
                       "scripts/confirm_ge_beam3_g3a.py", "docs/reference_cases/ge_beam3_g3a_confirmation_inventory.json",
                       "docs/GE_BEAM3_G3A_CORRECTION_CONFIRMATION.md",
                       "docs/GE_BEAM3_G3_GRAPH_JUNCTION_CONTRACT.md",
                       "docs/reference_cases/ge_beam3_g3_graph_contract_v1.json",
                       "docs/GE_BEAM3_G3A_REVIEW_STATUS.md",
                       "docs/reference_cases/ge_beam3_g3a_implementation_review_v1.json",
                       "docs/reference_cases/ge_beam3_g3_graph_fixtures_v1.json",
                       "tests/test_ge_beam3_g1_elastic.py", "tests/test_ge_beam3_g1_integration.py",
                       "tests/conftest.py", "conftest.py", "pyproject.toml",
                       "tests/test_ge_beam3_g3a_confirmation.py",
...
def worker(output):
    ...
    from anysolver._ge_beam3_g3_analysis import NativeGraphAnalysis
    from anysolver import _ge_beam3_g1_element as element
    phases = set()
    def mark(phase):
        phases.add(phase)
        print("G3A_CHECKPOINT "+phase, flush=True)
    ...
    with ExitStack() as stack:
        for target, name, phase in ((NativeGraphAnalysis, "_evaluate", "trial_assembly"),
                                    (NativeGraphAnalysis, "solve", "acceptance"),
                                    (element, "local_solve", "local_solve")):
            stack.enter_context(patch.object(target, name, instrument(getattr(target, name), phase)))
        stack.enter_context(patch.object(NativeGraphAnalysis, "resume",
                                         classmethod(instrument(NativeGraphAnalysis.resume.__func__, "restart"))))
...
def run_child(output):
    spec = importlib.util.spec_from_file_location("g3a_process", ROOT/"docs/reference_cases/e4_pl_s3_v2_bounded_process.py")
    module = importlib.util.module_from_spec(spec); sys.modules[spec.name] = module; spec.loader.exec_module(module)
    env = dict(os.environ)
    for name in THREADS: env[name] = "1"
    env.update(G3_PROGRESS="1", PYTHONPATH=str(ROOT/"src")+os.pathsep+str(ROOT), PYTHONDONTWRITEBYTECODE="1",
               PYTEST_DISABLE_PLUGIN_AUTOLOAD="1", PYTEST_ADDOPTS="",
               G3_DIAGNOSTIC_PAYLOAD=str(output/"packet.json"), G3_CORRECTION_RECORDS=str(output/"corrections"))
...
def main():
    ...
    expected = strict(INVENTORY.read_bytes().replace(b"\r\n", b"\n"))
    required_records = strict((ROOT/"docs/reference_cases/ge_beam3_g3a_correction_records.json").read_bytes().replace(b"\r\n", b"\n"))
    review = None
    ...
    started = time.monotonic(); packets = []; results = []; corrections = []
    try:
        for cycle in range(1, 2 if args.rehearse else 3):
            ...
            p = strict(packet, compact=True)
            if (set(p) != {"schema", "qualification", "inventory", "result", "recovery", "checkpoint_sha256"} or
                    p["schema"] != "GE_BEAM3_G3A_DEVELOPMENT_PACKET_V1" or p["qualification"] is not False):
                raise ValueError("unexpected diagnostic packet")
            if identity(args.candidate, clean=not args.rehearse) != frozen: raise ValueError("changed frozen inputs")
            entries = {}
            files = sorted((directory/"corrections").iterdir())
            if [p.name for p in files] != required_records or any(not p.is_file() or p.is_symlink() for p in files):
                raise ValueError("correction scientific inventory mismatch")
            for path in files:
                raw = path.read_bytes()
                # Test scientific records use the native compact canonical serializer.
                strict(raw, compact=True)
                entries[path.name] = dict(bytes=len(raw), sha256=sha256(raw).hexdigest())
            corrections.append(entries)
            packets.append(packet); results.append(body)
            print(f"PASS cycle {cycle}: {len(expected)} tests", flush=True)
        if (any(p != packets[0] for p in packets) or any(r != results[0] for r in results) or
                any(c != corrections[0] for c in corrections)):
            raise ValueError("nondeterministic confirmation")
        for name in required_records:
            if any((output/f"cycle-{i}"/"corrections"/name).read_bytes() !=
                   (output/"cycle-1"/"corrections"/name).read_bytes() for i in range(2, len(results)+1)):
                raise ValueError("nondeterministic confirmation")
        if time.monotonic()-started >= 1800: raise TimeoutError("wave deadline")
        if identity(args.candidate, clean=not args.rehearse) != frozen: raise ValueError("changed frozen inputs")
        aggregate = dict(schema="GE_BEAM3_G3A_CONFIRMATION_V1", candidate=args.candidate, tree=frozen["tree"],
                         inputs_sha256=sha256(canonical(frozen)).hexdigest(), review_sha256=args.review_sha256,
                         terminal="REHEARSAL_ONLY" if args.rehearse else "PROVISIONAL_GO_GE_BEAM3_G3A_NATIVE_GRAPH_ELASTIC_STATIC_ONLY",
                         scope="G3A_S15_S18_PRIVATE_NATIVE_ELASTIC_GRAPHS_ONLY", cycles=len(results),
                         tests=expected, packet=dict(bytes=len(packets[0]), sha256=sha256(packets[0]).hexdigest()),
                         correction_records=corrections[0], defaults_changed=False, general_static_parity=False)
        ...
        write(output/"failure.json", dict(status="BLOCKED_G3A_CONFIRMATION", exception=type(exc).__name__, message=str(exc)))
        raise
