...
def test_q1u_exact_backend_toy_cancellation_nested_radicals_inverse_rank_sign_and_serialization() -> None:
    root_text = os.environ.get("Q1U_EXACT_ENV_ROOT")
    assert root_text, "Q1U_EXACT_ENV_ROOT must name the frozen external exact environment"
    environment_root = Path(root_text).resolve(strict=True)
    assert environment_root.is_dir()

    ref1, raw_ref1 = _run(CASES / "e4_pl_q1u_reference.py")
    ref2, raw_ref2 = _run(CASES / "e4_pl_q1u_reference.py")
    ora1, raw_ora1 = _run(CASES / "e4_pl_q1u_oracle.py", environment_root)
    ora2, raw_ora2 = _run(CASES / "e4_pl_q1u_oracle.py", environment_root)
    assert raw_ref1 == raw_ref2 and raw_ora1 == raw_ora2

    for value, implementation_id in ((ref1, "Q1U_REFERENCE_STDLIB_FIELD_ALG"), (ora1, "Q1U_ORACLE_SYMPY_ALGEBRAIC_FIELD")):
