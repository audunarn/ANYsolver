...
def test_frozen_capacity_and_start():
    p.authority();assert p.request(request()) is None;p.receipt(receipt());assert p.TARGETS==(.01,.02,.03,.045)
...
def test_source_authority_mutation(tmp_path,monkeypatch):
    f=tmp_path/'docs/reference_cases/ge_beam3_retained_capacity_status.json';f.parent.mkdir(parents=True);f.write_bytes(b'{}\n')
