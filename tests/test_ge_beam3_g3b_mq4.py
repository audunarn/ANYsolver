"""Frozen M_Q4 development fixture; independent assembly, not formal acceptance."""
...
from anysolver._ge_beam3_g3b_q4_reference import Q4TranslationReferenceProblem, POLICY

ROOT = Path(__file__).resolve().parents[1]
FIXTURE = json.loads((ROOT/'docs/reference_cases/ge_beam3_g3_graph_fixtures_v1.json').read_text())
MQ4 = next(g for g in FIXTURE['mixed_graphs'] if g['id'] == 'M_Q4')
...
def parts():
    nodes = {n: np.array(x, dtype=float) for n, x in MQ4['nodes']}
    d = MQ4['native_element']; ids = tuple(d['nodes'])
    ...
    shell = QualifiedE4PLShellElement(MQ4['other_element']['id'], list(MQ4['other_element']['nodes']),
                                      thickness=m['thickness'], reference_normal=m['reference_normal'])
    material = Material('G3b frozen shell', m['E'], m['nu'])
    return native, shell, material, dict(nodes=nodes, fixed_nodes=tuple(MQ4['fixed_nodes']),
        tie=deepcopy(MQ4['tie']), reference_normal=deepcopy(m['reference_normal']))



def make():
    native, shell, material, kw = parts()
    return Q4TranslationReferenceProblem(native, shell, material, **kw)


def load(p, *, moment=True):
    f = np.zeros(42); i = p.ids.index(MQ4['load_node'])*6
...
def test_mq4_reference_kkt_smoke():
...
def test_rejection_precedes_both_element_operators(case):
    native, shell, material, kw = parts(); cls = Q4TranslationReferenceProblem
    if case == 'policy': kw['policy'] = 'FINITE'
        ...
        class Unregistered(Q4TranslationReferenceProblem): pass
        cls = Unregistered
...
def test_mq4_deterministic_development_packet(tmp_path):
    a, b = make(), make(); f = load(a)
    packet = canonical(dict(schema='G3B_M_Q4_DEVELOPMENT_V1', qualification=False, result=a.solve(f)))
    assert packet == canonical(dict(schema='G3B_M_Q4_DEVELOPMENT_V1', qualification=False, result=b.solve(f)))
    path = Path(os.environ.get('G3B_MQ4_DIAGNOSTIC_PAYLOAD', str(tmp_path/'mq4.json')))
...
def test_shuffle_insertion_and_rhs_reuse():
    ...
    other = Q4TranslationReferenceProblem(n,s,m,**kw)
...
def test_mq4_extent_and_frozen_parent_bindings():
    import hashlib
    import subprocess
    parent = 'd1a564857b2b74f8b0d53c4028040a31652b9caa'
    ...
    allowed = {
        ...
        'tests/test_ge_beam3_g3b_mb3.py':'M',
    ...
    for b in contract['source_bindings']+contract['text_bindings']:
        raw=(ROOT/b['path']).read_bytes().replace(b'\r\n',b'\n')
        assert len(raw) == b['bytes'] and hashlib.sha256(raw).hexdigest() == b['sha256']
