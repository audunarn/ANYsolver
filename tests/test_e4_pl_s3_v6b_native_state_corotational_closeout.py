...
def test_v6b_closeout_binds_the_immutable_authority_and_exact_extent() -> None:
    _result_raw, result = _canonical(
        "e4_pl_s3_v6b_v2d_native_state_corotational_result.json"
    )
    ...
    _contract_raw, contract = _canonical(
        "e4_pl_s3_v6b_v2d_native_state_corotational_contract.json"
    )
    assert sorted(paths) == sorted(contract["exact_implementation_extent"])
    assert len(paths) == authority["exact_implementation_paths"] == 9


def test_v6b_result_has_two_clean_cycles_and_only_successor_authority() -> None:
    _raw, result = _canonical(
        "e4_pl_s3_v6b_v2d_native_state_corotational_result.json"
    )
    assert result["cycles"] == {"count": 2, "tests_passed_per_cycle": 54}
    assert all(result["checks"].values())
    assert result["terminal"] == (
        "PROVISIONAL_GO_E4_PL_S3_V6B_OFFSET_LOAD_RESTART_INTEGRATION"
    )
    assert result["next_gate"] == "V6C_V2D_OFFSET_LOAD_RESTART_INTEGRATION"
    assert result["activation_authorized"] is False
    assert result["solver_integrated_hot_restart_authorized"] is False
...
def test_v6b_review_is_five_key_empty_p0_p1_and_status_binds_bytes() -> None:
    result_raw, result = _canonical(
        "e4_pl_s3_v6b_v2d_native_state_corotational_result.json"
    )
    review_raw, review = _canonical(
        "e4_pl_s3_v6b_v2d_native_state_corotational_review.json"
    )
    _status_raw, status = _canonical(
        "e4_pl_s3_v6b_v2d_native_state_corotational_status.json"
    )
    assert set(review) == {
        "conclusions",
        "findings",
