...
def test_reassembled_modes_wheel_two_fresh_processes(tmp_path):
    ...
    with source_map.open('xb') as stream:
        stream.write(canonical(dict(schema='GE_BEAM3_REASSEMBLED_MODES_INSTALLED_SOURCE_MAP_V1',
            candidate_commit='a36baf8be16f4cac0c2c8f43060e66493bdafa49',
            candidate_tree='2fcba00faf7ab4077d24dd00426e38c3b6b29af2',
    ...
    def command(argv, cwd, name, limit=180):
        print('reassembled-mode wheel: '+name+' started', flush=True)
        ...
        print('reassembled-mode wheel: '+name+' completed', flush=True)
    ...
    candidate = 'a36baf8be16f4cac0c2c8f43060e66493bdafa49'
    ...
    with zipfile.ZipFile(wheel) as archive:
        assert all('anysolver/'+name in archive.namelist() for name in (
            '_native_reassembled_factor_modes.py', '_ge_beam3_reassembled_signed_modes.py', '_dyadic_bilinear.py'))
    ...
    shutil.copy2(records/'ge_beam3_reassembled_modes_installed_smoke.py', script)
    results = []; diagnostics = []
    for index in (1, 2):
    ...
    assert len(result['cases']) == 10 and result['curved_covariance_passed']
    assert len(result['source_files']) == 83 and not result['production_qualified']
    receipt = dict(schema='GE_BEAM3_REASSEMBLED_MODES_PACKAGE_TEST_RECEIPT_V1',
