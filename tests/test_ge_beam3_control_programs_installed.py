"""Offline fresh-venv correctness of private displacement and arc controls."""
...
def test_control_programs_wheel_two_fresh_processes(tmp_path):
    ...
    with source_map.open('xb') as stream:
        stream.write(canonical(dict(schema='GE_BEAM3_CONTROL_PROGRAMS_INSTALLED_SOURCE_MAP_V1',
            candidate_commit='f7c65a157bafc37ee9bd6cefbceff151710a1c62',
    ...
    with zipfile.ZipFile(wheel) as archive:
        for name in ('_ge_beam3_displacement_program.py', '_ge_beam3_arc_geometry.py', '_ge_beam3_arc_program.py'):
    ...
    shutil.copy2(records/'ge_beam3_control_programs_installed_smoke.py', script)
    results = []; checkpoints = []
    for index in (1, 2):
    ...
    assert len(result['source_files']) == len(outputs) and len(outputs) >= 55
    ...
    receipt = dict(schema='GE_BEAM3_CONTROL_PROGRAMS_PACKAGE_TEST_RECEIPT_V1',
