...
def test_v6c_closeout_binds_the_immutable_authority_and_exact_extent() -> None:
    _result_raw, result = _canonical(
        "e4_pl_s3_v6c_v2d_offset_load_restart_result.json"
    )
    ...
    contract_raw, contract = _canonical(
        "e4_pl_s3_v6c_v2d_offset_load_restart_contract.json"
    )
    assert sorted(paths) == sorted(contract["exact_implementation_extent"])
    assert len(paths) == authority["exact_implementation_paths"] == 10
    assert authority["contract_sha256"] == _sha(contract_raw)


def test_v6c_result_has_two_clean_cycles_and_only_successor_authority() -> None:
    _raw, result = _canonical(
        "e4_pl_s3_v6c_v2d_offset_load_restart_result.json"
    )
    assert result["cycles"] == {"count": 2, "tests_passed_per_cycle": 32}
    assert all(result["checks"].values())
    assert result["terminal"] == (
        "PROVISIONAL_GO_E4_PL_S3_V6C_REMAINING_PARITY"
    )
    assert result["next_gate"] == "V6D_V2D_ACTIVITY_CONTACT_BATCH_PARITY"
    assert result["activation_authorized"] is False
    assert result["solver_integrated_hot_restart_authorized"] is True
...
def test_v6c_review_is_five_key_empty_p0_p1_and_status_binds_bytes() -> None:
    result_raw, result = _canonical(
        "e4_pl_s3_v6c_v2d_offset_load_restart_result.json"
    )
    review_raw, review = _canonical(
        "e4_pl_s3_v6c_v2d_offset_load_restart_review.json"
    )
    _status_raw, status = _canonical(
        "e4_pl_s3_v6c_v2d_offset_load_restart_status.json"
    )
    assert set(review) == {
        "conclusions",
        "findings",
