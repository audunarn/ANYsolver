...
from docs.reference_cases import ge_beam3_curved_p5_centered_chord32_diagnostic as diagnostic
...
@pytest.mark.parametrize('mutation', ['head', 'dirty', 'extent', 'parent', 'sibling', 'environment', 'failure_input', 'force_input'])
...
def test_worker_failure_preserves_logs_without_diagnostic_publication(tmp_path, monkeypatch, failure):
    ...
    assert (output/'blocked-diagnostic.json').exists() and (output/'centered-chord32/partial.log').read_text() == 'preserved'
...
@pytest.mark.parametrize('request_id', ['69778aebc5d44cbd8f91e0426edc2232',
    '5b59e2613245422a922616891f8b003b', '5fbe008bfc7744079ed0e35d12dc72f1'])
