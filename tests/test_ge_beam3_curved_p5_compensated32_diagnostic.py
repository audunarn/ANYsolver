...
from docs.reference_cases.ge_beam3_curved_p5_compensated_control import CompensatedDisplacementControlledAssemblyProbe as DisplacementControlledAssemblyProbe
...
@pytest.mark.parametrize('mutation', ['head', 'dirty', 'extent', 'parent', 'sibling', 'environment', 'failure_input', 'force_input', 'centered_input'])
def test_frozen_identity_mutations(monkeypatch, tmp_path, mutation):
    ...
    def force():
        if mutation == 'force_input': raise ValueError('changed force input')
        return {}
    monkeypatch.setattr(diagnostic, 'force_inputs', force)
    def centered():
        if mutation == 'centered_input': raise ValueError('changed centered input')
        return {}
    monkeypatch.setattr(diagnostic, 'centered_inputs', centered)
...
def test_worker_failure_preserves_logs_without_diagnostic_publication(tmp_path, monkeypatch, failure):
    ...
    assert (output/'blocked-diagnostic.json').exists() and (output/'compensated32/partial.log').read_text() == 'preserved'
...
def test_preserved_historical_bindings_are_correct_read_only():
    ...
    assert len(diagnostic.force_inputs()) == 10
    assert len(diagnostic.centered_inputs()) == 10


@pytest.mark.parametrize('request_id', ['69778aebc5d44cbd8f91e0426edc2232',
    '5b59e2613245422a922616891f8b003b', '5fbe008bfc7744079ed0e35d12dc72f1', '1b6bdcd793404eae8d19435b708701d7'])
def test_known_consumed_requests_rejected_even_with_pending_ledger_accounting(tmp_path, request_id):
    manager = tmp_path/'manager';(manager/'requests').mkdir(parents=True);(manager/'active-lock').mkdir()
    repo = tmp_path/'repo';repo.mkdir();output = tmp_path/'output';commit = 'a'*40
    record = {'request_id': request_id, 'repository': str(repo), 'command': diagnostic.command(repo, commit, output)}
    diagnostic.shared.publish(manager/'requests'/(request_id+'.json'), record)
    diagnostic.shared.publish(manager/'active-lock/owner.json', record)
    (manager/'ledger.md').write_text(f'| now | {request_id} | APPROVED | test |\n')
    with pytest.raises(ValueError, match='historical request remains consumed'):
        diagnostic.lease(repo, commit, output, manager=manager)


def test_force_input_hash_and_inventory_mutations_rejected(tmp_path, monkeypatch):
    data = b'preserved\n';path = tmp_path/'record.json';path.write_bytes(data)
    monkeypatch.setattr(diagnostic, 'FORCE_ROOT', tmp_path)
    monkeypatch.setattr(diagnostic, 'FORCE', {'record.json': (len(data), diagnostic.shared.sha(data))})
    assert len(diagnostic.force_inputs()) == 1
    path.write_bytes(b'mutated!!\n')
    with pytest.raises(ValueError, match='hash mismatch'): diagnostic.force_inputs()
    path.write_bytes(data);(tmp_path/'extra.json').write_bytes(b'{}\n')
    with pytest.raises(ValueError, match='inventory changed'): diagnostic.force_inputs()


@pytest.mark.parametrize('mutation', ['missing_low','coordinate_schema','low_mismatch','element_low','clamp_low','control_low','normalization'])
def test_compensated_coordinate_mutations_rejected_after_rehash(tmp_path,captures,mutation):
    folder,made,_=packet(tmp_path,captures,True)
    path=folder/'failed_last.json';raw=diagnostic.shared.load(path)
    if mutation=='missing_low': del raw['position_low']
    if mutation=='coordinate_schema': raw['coordinate_schema']='old'
    if mutation=='low_mismatch': raw['position_low'][1][0]=0. if raw['position_low'][1][0] else 2.**-65
    if mutation=='element_low': raw['response']['elements'][0]['position_low'][1][0]=1.
    if mutation=='clamp_low': raw['position_low'][0][0]=2.**-65
    if mutation=='control_low': raw['position_low'][2][1]=2.**-65
    if mutation=='normalization': raw['position_low'][1][0]=1.
    data=diagnostic.shared.canonical(raw);path.write_bytes(data)
    made['result']['bindings']['failed_last']={'name':path.name,'bytes':len(data),'sha256':diagnostic.shared.sha(data)}
    (folder/'complete.json').write_bytes(diagnostic.shared.canonical(made))
    with pytest.raises(ValueError): diagnostic.inspect(folder,{},'a'*32,'b'*64,count=2)
