"""Frozen M_Q4_WEIGHTED development fixture; independent assembly, not formal acceptance."""
...
from anysolver._ge_beam3_g3b_weighted_reference import WeightedQ4TranslationReferenceProblem, POLICY

ROOT = Path(__file__).resolve().parents[1]
FIXTURE = json.loads((ROOT/'docs/reference_cases/ge_beam3_g3_graph_fixtures_v1.json').read_text())
WEIGHTED = next(g for g in FIXTURE['mixed_graphs'] if g['id'] == 'M_Q4_WEIGHTED')
...
def parts():
    nodes = {n: np.array(x, dtype=float) for n, x in WEIGHTED['nodes']}
    d = WEIGHTED['native_element']; ids = tuple(d['nodes'])
    ...
    shell = QualifiedE4PLShellElement(WEIGHTED['other_element']['id'], list(WEIGHTED['other_element']['nodes']),
                                      thickness=m['thickness'], reference_normal=m['reference_normal'])
    material = Material('G3b frozen shell', m['E'], m['nu'])
    return native, shell, material, dict(nodes=nodes, fixed_nodes=tuple(WEIGHTED['fixed_nodes']),
        tie=deepcopy(WEIGHTED['tie']), reference_normal=deepcopy(m['reference_normal']))



def make():
    native, shell, material, kw = parts()
    return WeightedQ4TranslationReferenceProblem(native, shell, material, **kw)


def load(p, *, moment=True):
    f = np.zeros(42); i = p.ids.index(WEIGHTED['load_node'])*6
...
def independent_full_reference():
    ...
    native_all = native_dofs+list(range(42, 66))
...
def test_weighted_reference_kkt_smoke():
    ...
    invariant(result['u'], ref[:42]); invariant(result['internal'], ref[42:66])
    invariant(result['multipliers'], ref[66:]); invariant(p.J, J[:, :42])
    invariant(result['energy'], ref[:66] @ H @ ref[:66]/2)
    invariant(result['energy']*2, result['u'] @ f)
    force = result['support_reactions'].reshape(7, 6)+f.reshape(7, 6)
    invariant(force[:, :3].sum(axis=0), np.zeros(3))
    invariant((force[:, 3:]+np.cross(p.positions, force[:, :3])).sum(axis=0), np.zeros(3))
    tie = result['tie_forces'].reshape(7, 6)
    invariant(tie[:, :3].sum(axis=0), np.zeros(3)); assert not np.any(tie[:, 3:])
    for node in (201,202,203,204): invariant(tie[p.ids.index(103)], -4*tie[p.ids.index(node)])
    invariant(np.cross(p.positions, tie[:,:3]).sum(axis=0), np.zeros(3))
    assert len(result['native_stations']) == 8
    expected = native.operator.cell.recover(ref[48:66])
    ...
    assert result['shell_recovery']['physical_director_authoritative'] is True
    invariant(result['shell_recovery']['physical_director'], [0, 0, 1])
    assert p.T.shape == (42, 21) and system.shape == (87, 87)
...
def test_translation_tie_has_no_rotational_work_or_equality():
    p = make(); f = np.zeros(42); a = 6*p.ids.index(103); b = 6*p.ids.index(201)
    f[a+3] = FIXTURE['programs']['mixed_moment'][0]
    r = p.solve(f)
    assert abs(r['u'][a+3]) > 1e-8 and abs(r['u'][b+3]) <= 1e-11
    assert not np.any(p.J[18:, np.array([6*i+j for i in range(7) for j in (3, 4, 5)])])
...
def test_rejection_precedes_both_element_operators(case):
    native, shell, material, kw = parts(); cls = WeightedQ4TranslationReferenceProblem
    ...
    elif case == 'shared_node': shell.node_ids = (103, 202, 203, 204)
    ...
    elif case == 'geometry': kw['nodes'][204][2] = .01
    elif case == 'subclass':
        class Unregistered(WeightedQ4TranslationReferenceProblem): pass
        cls = Unregistered
    elif case == 'wrong_class':
        from anysolver.elements import ShellElement
        shell = ShellElement(2, [201,202,203,204])
    elif case == 'thickness': shell.thickness = .3
    else: object.__setattr__(shell, '_qualified_components', {})
    with patch.object(ElasticOperator, 'evaluate', side_effect=AssertionError('native mechanics called')):
        with patch.object(QualifiedE4PLShellElement, 'compute_stiffness_matrix', side_effect=AssertionError('shell mechanics called')):
...
def test_frozen_capture_mutations_rejected(mutation):
    ...
    elif mutation == 'operator': p.K = p.K+np.eye(42)
...
def test_q4_numerical_drill_fields_excluded_from_physical_recovery():
    p = make(); result = p.solve(load(p)); u = result['u'][list(p._dofs[1])].copy()
    u[[5,11,17,23]] += np.array([.1,-.2,.3,-.4])
...
def test_weighted_deterministic_development_packet(tmp_path):
    a, b = make(), make(); f = load(a)
    packet = canonical(dict(schema='G3B_M_Q4_WEIGHTED_DEVELOPMENT_V1', qualification=False, result=a.solve(f)))
    assert packet == canonical(dict(schema='G3B_M_Q4_WEIGHTED_DEVELOPMENT_V1', qualification=False, result=b.solve(f)))
    path = Path(os.environ.get('G3B_WEIGHTED_DIAGNOSTIC_PAYLOAD', str(tmp_path/'weighted.json')))
...
def test_shuffle_insertion_and_rhs_reuse():
    ...
    other = WeightedQ4TranslationReferenceProblem(n,s,m,**kw)
...
def test_exact_rational_trace_and_constraint_elimination():
    from fractions import Fraction as F
    p = make(); masters = [p.ids.index(n) for n in (201,202,203,204)]; slave = p.ids.index(103)
    points = [[F(float(v)) for v in point] for point in p.positions]
    weights = [F(w) for _,w in WEIGHTED['tie']['masters']]
    assert weights == [F(1,4)]*4 and sum(weights) == 1
    assert [sum(w*points[n][d] for w,n in zip(weights,masters)) for d in range(3)] == points[slave]
    assert points[slave] == [F(1,2),F(1,2),F(0)]
    # Exact reproduction of a general affine translation field, not only
    # constant translation. No rotational equality is part of the map.
    A = [[F(2),F(-3),F(1,2)], [F(-1),F(4),F(2)], [F(3),F(1,3),F(-2)]]
    def field(x): return [F(d+1,7)+sum(A[d][j]*x[j] for j in range(3)) for d in range(3)]
    assert [sum(w*field(points[n])[d] for w,n in zip(weights,masters)) for d in range(3)] == field(points[slave])
    J = [[F(float(v)) for v in row] for row in p.J]
    T = [[F(float(v)) for v in row] for row in p.T]
    assert all(sum(J[i][k]*T[k][j] for k in range(42)) == 0 for i in range(21) for j in range(21))
    assert all(J[i][6*n+d] == 0 for i in range(18,21) for n in range(7) for d in (3,4,5))


@pytest.mark.parametrize('axis', [0,1,2])
def test_weighted_force_moment_and_dual_work(axis):
    p = make(); f = np.zeros(42); f[6*p.ids.index(103)+axis] = .002
    r = p.solve(f); forces = r['tie_forces'].reshape(7,6)
    corners = [p.ids.index(n) for n in (201,202,203,204)]; tip = p.ids.index(103)
    assert np.linalg.norm(forces[tip,:3]) > 1e-8
    for corner in corners: invariant(forces[corner,:3], -forces[tip,:3]/4)
    # Both supported corner rows retain their force share in the full dual map.
    assert np.linalg.norm(forces[p.ids.index(202),:3]) > 1e-8
    assert np.linalg.norm(forces[p.ids.index(203),:3]) > 1e-8
    invariant(np.cross(p.positions,forces[:,:3]).sum(axis=0), np.zeros(3))
    delta = (p.T @ np.linspace(-.125,.375,21)).reshape(7,6)
    beam_work = delta[tip,:3] @ forces[tip,:3]
    shell_work = sum(delta[n,:3] @ forces[n,:3] for n in corners)
    invariant(beam_work+shell_work,0.)
    invariant(delta[tip,:3],delta[corners,:3].sum(axis=0)/4)


@pytest.mark.parametrize('case', ['missing_master','duplicate_master','foreign_master','float_weight',
    'equivalent_fraction','unit_but_wrong_position','centre_but_nonregistered_weights','off_centre'])
def test_weighted_authority_rejects_before_evaluation(case):
    native,shell,material,kw = parts()
    masters = kw['tie']['masters']
    if case == 'missing_master': masters.pop()
    elif case == 'duplicate_master': masters[1][0] = masters[0][0]
    elif case == 'foreign_master': masters[1][0] = 999
    elif case == 'float_weight': masters[0][1] = .25
    elif case == 'equivalent_fraction': masters[0][1] = '2/8'
    elif case == 'unit_but_wrong_position':
        for m,w in zip(masters,('1/2','1/2','0','0')): m[1] = w
    elif case == 'centre_but_nonregistered_weights':
        for m,w in zip(masters,('1/2','0','1/2','0')): m[1] = w
    else:
        for n in native.node_ids: kw['nodes'][n][1] += .125
        native = ElasticElement(native.element_id,native.node_ids,
            Reference([kw['nodes'][n] for n in native.node_ids],np.tile(np.eye(3),(3,1,1))),
            ElasticSection.isotropic(**FIXTURE['materials']['native_isotropic']))
    with patch.object(ElasticOperator,'evaluate',side_effect=AssertionError('native mechanics called')):
        with patch.object(QualifiedE4PLShellElement,'compute_stiffness_matrix',side_effect=AssertionError('shell mechanics called')):
            with pytest.raises(ValueError): WeightedQ4TranslationReferenceProblem(native,shell,material,**kw)


def test_weighted_extent_and_frozen_parent_bindings():
    import hashlib
    import subprocess
    parent = 'df9a68310943b2e8455d2a06905bd714321bee14'
    git = ['git','-c','safe.directory='+ROOT.as_posix()]
    allowed = {'src/anysolver/_ge_beam3_g3b_weighted_reference.py':'A',
        'tests/test_ge_beam3_g3b_weighted.py':'A','docs/GE_BEAM3_G3B_WEIGHTED_DEVELOPMENT.md':'A',
        'docs/reference_cases/ge_beam3_g3b_weighted_development_v1.json':'A',
        ...
        'tests/test_ge_beam3_g3b_ms3.py':'M'}
    ...
    markers = {'mb2':'test_additive_scope_and_immutable_parent_authority',
    ...
    for lane,name in markers.items():
        path = 'tests/test_ge_beam3_g3b_'+lane+'.py'; marker = ('def '+name+'():').encode()
