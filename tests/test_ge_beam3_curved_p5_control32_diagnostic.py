...
from docs.reference_cases import ge_beam3_curved_p5_control32_diagnostic as diagnostic
...
def test_worker_failure_preserves_logs_without_diagnostic_publication(tmp_path, monkeypatch, failure):
    ...
    assert (output/'blocked-diagnostic.json').exists() and (output/'control32/partial.log').read_text() == 'preserved'
