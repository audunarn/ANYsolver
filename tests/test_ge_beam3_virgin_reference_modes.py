...
from anysolver._ge_beam3_virgin_reference_modes import solve_virgin_reference_modes
...
def test_native_virgin_reference_uses_retained_factors(slenderness, tmp_path):
    ...
    packet, modes = solve_virgin_reference_modes(model, states, np.zeros(18),
    ...
    with (tmp_path/'native-reference.json').open('xb') as stream:
...
def test_nonvirgin_or_invalid_inputs_cannot_enter_reference_kernel(kind, monkeypatch):
    import anysolver._ge_beam3_virgin_reference_modes as adapter
    ...
    monkeypatch.setattr(adapter, 'solve_reference_factor_spectrum', forbidden)
    with pytest.raises((ValueError, np.linalg.LinAlgError)):
        solve_virgin_reference_modes(model, states, total, inertias)


def test_model_changed_inside_reference_solver_is_rejected_after_return(monkeypatch):
    import anysolver._ge_beam3_virgin_reference_modes as adapter
    model = problem(); states = initial(model); solve = adapter.solve_reference_factor_spectrum
    def changed(*args, **kwargs):
        result = solve(*args, **kwargs)
        model.mesh.dof_manager._total_dofs += 6
        return result
    monkeypatch.setattr(adapter, 'solve_reference_factor_spectrum', changed)
    with pytest.raises(ValueError):
        solve_virgin_reference_modes(model, states, np.zeros(18), {1: np.eye(6)})
...
def test_rotated_curved_reference_frequencies_and_factors_agree():
    ...
    results = [solve_virgin_reference_modes(m, initial(m), np.zeros(18), {1: np.eye(6)}) for m in (old, new)]
