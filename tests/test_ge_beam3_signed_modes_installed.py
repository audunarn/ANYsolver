...
def test_signed_modes_wheel_two_fresh_processes(tmp_path):
    ...
    with source_map.open('xb') as stream:
        stream.write(canonical(dict(schema='GE_BEAM3_SIGNED_MODES_INSTALLED_SOURCE_MAP_V1',
            candidate_commit='2b0ee70beda6f12e81bb57aa95195c424364a23d',
            candidate_tree='f8a21473610c6e05abe92847f796c0ea80ff88b2',
    ...
    def command(argv, cwd, name, limit=180):
        print('signed-mode wheel: '+name+' started', flush=True)
        ...
        print('signed-mode wheel: '+name+' completed', flush=True)
    ...
    candidate = '2b0ee70beda6f12e81bb57aa95195c424364a23d'
    ...
    with zipfile.ZipFile(wheel) as archive:
        assert all('anysolver/'+name in archive.namelist() for name in (
            '_native_signed_factor_modes.py', '_ge_beam3_signed_loaded_modes.py'))
    ...
    shutil.copy2(records/'ge_beam3_signed_modes_installed_smoke.py', script)
    results = []; diagnostics = []
    for index in (1, 2):
    ...
    assert len(result['source_files']) == 67 and not result['production_qualified']
    receipt = dict(schema='GE_BEAM3_SIGNED_MODES_PACKAGE_TEST_RECEIPT_V1',
