"""One frozen, process-contained curved dead-line development wave; no retry."""
import argparse
...
def summary(output,revision):
    from docs.reference_cases.ge_beam3_dead_line_comparison import recompute
    ...
    return dict(schema='GE_BEAM3_CURVED_DEAD_LINE_DEVELOPMENT_V1',revision=revision,rows=rows,artifacts=artifacts,
...
def worker(revision,output):
    ...
    from docs.reference_cases.ge_beam3_dead_line_comparison import build
...
def run(revision,output):
    ...
    command=[sys.executable,'-B','-m','docs.reference_cases.ge_beam3_dead_line_probe',
    ...
    try:
        with (output/'stdout.log').open('xb') as stdout,(output/'stderr.log').open('xb') as stderr:
            ...
            while True:
                ...
                if now-start>=600 or now-activity>=120 or memory>=24*(1<<30):
                    raise RuntimeError('curved dead-line wall/inactivity/memory bound')
                if code is not None and active==0: break
                time.sleep(.2)
            if code!=0: raise RuntimeError(f'curved dead-line child failed: exit {code}; no retry')
        ...
        try:
            if not job.terminate(): raise RuntimeError('complete curved dead-line process cleanup unproven')
        finally: job.close()
    guard(revision)
