"""New six-macro diagnostic; never retries the preserved four-macro request.

Fresh resource approval/lock required externally. The saved nominal reference
loads are read and hash-checked; no BVP/reference generation is performed.
"""
import argparse
...
def ready(checkpoint_raw, reference_raw, revision):
    ...
    rows = audit.inspect_records(audit.parse(checkpoint_raw), audit.parse(reference_raw), macros=6)
    return dict(schema='GE_BEAM3_SIX_MACRO_PRESERVED_REFERENCE_LOAD_DIAGNOSTIC_V1',
        status='DEVELOPMENT_LOAD_COMPARISON_ONLY', revision=revision, macros=6,
...
def worker(revision, output):
    ...
    progress(dict(stage='INITIALIZATION', macros=6))
    model = family.model(6); program = family.program(6)
    result = control.solve_translation_program(model, program, progress=progress)
    ...
    control.Context(model, program).restore(result.checkpoint,
        expected_sha256=sha256(result.checkpoint).hexdigest())
    value = ready(result.checkpoint, reference, revision)
    guard(revision)
...
def run(revision, output):
    ...
    try:
        with (output/'stdout.log').open('xb') as out, (output/'stderr.log').open('xb') as err:
            ...
            while True:
                ...
                if now-started >= 600 or now-activity >= 120 or memory >= 24*(1 << 30):
                    raise RuntimeError('six-macro wall/inactivity/memory bound')
                if code is not None and active == 0: break
                time.sleep(.2)
            if code != 0: raise RuntimeError(f'six-macro diagnostic failed: exit {code}')
        guard(revision)
        ...
        expected = audit.canonical(ready(checkpoint.read_bytes(), reference, revision))
