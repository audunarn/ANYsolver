#!/usr/bin/env python3
"""Mechanics-free guarded launcher for the Q1V scientific inventory.
...
RESULT_SCHEMA = "anysolver.s4.e4-pl-q1v-scientific-test-result-v1"
BLOCKED_TERMINAL = "BLOCKED_E4_PL_Q1V_CONTRACT_OR_NONDETERMINISM"
...
REVIEW_EXPECTATIONS = {
    "plan": (
        "anysolver.s4.e4-pl-q1v-plan-review-v1",
        "ACCEPT_Q1V_PREREGISTRATION_NO_P0_P1",
    ),
    "implementation": (
        "anysolver.s4.e4-pl-q1v-implementation-review-v1",
        "ACCEPT_Q1V_IMPLEMENTATION_FREEZE_NO_P0_P1",
    ),
    "contract": (
        "anysolver.s4.e4-pl-q1v-contract-review-v1",
        "ACCEPT_Q1V_EXECUTION_CONTRACT_NO_P0_P1",
    ),
}
REVIEW_PATHS = {
    "plan": "docs/reference_cases/e4_pl_q1v_plan_review.json",
    "implementation": "docs/reference_cases/e4_pl_q1v_implementation_review.json",
    "contract": "docs/reference_cases/e4_pl_q1v_contract_review.json",
}
...
class AuthorityError(RuntimeError):
    """Raised before collection when Q1V execution authority is invalid."""
...
def _repo_root() -> Path:
    ...
    if completed.returncode:
        raise AuthorityError("runner is not in the Q1V Git worktree")
...
def _verify_inherited_rows(root: Path, inherited: Mapping[str, Any]) -> None:
    ...
    rows = inherited["rows"]
    if int(inherited["count"]) != 117 or not isinstance(rows, list) or len(rows) != 117:
        raise AuthorityError("Q1V must bind exactly 117 inherited rows")
    seen: set[str] = set()
    for row in rows:
    ...
    expected_rows = manifest["inputs"]
    if rows != expected_rows:
        raise AuthorityError("contract does not bind the exact expanded 117-row inheritance")
...
def _verify_stage_bindings(root: Path, contract: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise AuthorityError("implementation inputs do not bind exact IMPLEMENTATION20")
...
def verify_authority(
    ...
    """Authenticate Q1V execution without importing or collecting mechanics."""
    ...
    if contract_resolved != committed_contract:
        raise AuthorityError("contract path is not the committed Q1V contract")
    ...
    if environment.get("candidate_id") != "candidate_e4_pl_q1t.wg2020_numbered_frame_surface_pl_planar_linear_iso_v1":
        raise AuthorityError("frozen Q1T environment identity mismatch")
    ...
    if base != Q1U_CLOSEOUT:
        raise AuthorityError("wrong fixed Q1S closeout ancestry")
    for commit, parent in ((head, commit2), (commit2, commit1), (commit1, Q1U_CLOSEOUT)):
        if _run_git(root, "rev-list", "--parents", "-n", "1", commit).split() != [commit, parent]:
            raise AuthorityError("Q1V authority commits must have exactly one parent")
    ancestry = contract["commit_ancestry"]
    _require_exact_keys(ancestry, {"commit1", "commit2"}, "commit ancestry")
    _verify_commit_row(root, ancestry["commit1"], commit1, Q1U_CLOSEOUT, COMMIT1_SUBJECT, PLAN_PATHS, "Commit 1")
    ...
    if terminal.get("path") != "docs/reference_cases/e4_pl_q1v_terminal_table.json":
        raise AuthorityError("wrong terminal-table path")
    if terminal.get("schema") != "anysolver.s4.e4-pl-q1v-terminal-table-v1":
    ...
    if terminal.get("terminal_count") != 12:
        raise AuthorityError("wrong Q1V terminal count")
...
def _execute_pytest(
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="anysolver-q1v-scientific-"))
    worktrees = _worktree_roots(root)
