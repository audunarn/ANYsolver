"""Independent exact reference for the E4-PL-Q1U local qualification.
...
PLAN_COMMIT = "2404ec3cec03fe9ddef131d9bfd39a24e4e7eabc"
PLAN_TREE = "25bc45287495e9349eeebf552e76f88ec70c13b6"
PLAN_SUBJECT = "docs: preregister E4 PL Q1U execution-guard completion"
IMPLEMENTATION_SUBJECT = "docs: freeze E4 PL Q1U guard-corrected implementations"
EXECUTION_SUBJECT = "docs: authorize E4 PL Q1U scientific execution"
EXECUTION_TOKEN = "AUTHORIZE_E4_PL_Q1U_SCIENTIFIC_EXECUTION"
PAYLOAD_SCHEMA = "e4_pl_q1u_common_certificate_payload_v1"
OUTPUT_SCHEMA = "anysolver.s4.e4-pl-q1u-reference-raw-v1"
...
class Field:
    ...
    def with_sqrt(self, value: "Alg") -> tuple["Field", "Alg"]:
        ...
        if len(self.radicands) >= 5:
            raise ValueError("frozen Q1U exact field exceeds formal degree 32")
...
def _contracts() -> dict[str, object]:
    ...
    manifest = result["e4_pl_q1u_inheritance_manifest"]
    if not isinstance(manifest, dict):
        raise ValueError("Q1U inheritance manifest must be an object")
    direct_rows = manifest["inputs"]  # type: ignore[index]
    if len(direct_rows) != 82 or len({str(row["path"]) for row in direct_rows}) != 82:
        raise ValueError("Q1U inheritance manifest must bind exactly 82 distinct paths")
...
def static_transcription() -> dict[str, object]:
    ...
    if any(offset < 0 for offset in offsets) or offsets != sorted(offsets):
        raise AssertionError("Q1U g1..g5 generator schedule is absent or reordered")
    ...
    return {
        "schema": "anysolver.s4.e4-pl-q1u-reference-static-transcription-v1",
...
def toy_exact_backend_certificate() -> dict[str, object]:
    ...
    payload = {
        "schema": "anysolver.s4.e4-pl-q1u-reference-toy-exact-backend-v1",
        "candidate_id": CANDIDATE_ID,
        "implementation_id": IMPLEMENTATION_ID,
        "domain_equalities": {
...
def _stage_paths(stage: str) -> list[str]:
    allowed = load_json(ROOT / "docs/reference_cases/e4_pl_q1u_allowed_extent.json")
...
def _validate_execution_contract(path: pathlib.Path, expected_sha256: str) -> dict[str, object]:
    expected_path = (ROOT / "docs/reference_cases/e4_pl_q1u_execution_contract.json").resolve()
    path = path.resolve()
    ...
    authority_contract = load_json(ROOT / "docs/reference_cases/e4_pl_q1u_authority_contract.json")
    ...
    environment_path = _bound_path(
        ...
        expected_path="docs/reference_cases/e4_pl_q1u_environment.json",
    )
    environment_record = load_json(environment_path)
    if environment != {
        "record_path": "docs/reference_cases/e4_pl_q1u_environment.json",
    ...
    inheritance_manifest = load_json(ROOT / "docs/reference_cases/e4_pl_q1u_inheritance_manifest.json")
    ...
    implementations = {
        "reference": (
            ...
            "Q1U_REFERENCE_STDLIB_FIELD_ALG",
        ),
        "oracle": (
            "docs/reference_cases/e4_pl_q1u_oracle.py",
            "Q1U_ORACLE_SYMPY_ALGEBRAIC_FIELD",
        ),
    }
    ...
    _bound_path(
        ...
        expected_path="docs/reference_cases/e4_pl_q1u_scientific_test_runner.py",
    )
    ...
    manifest_path = _bound_path(
        ...
        expected_path="docs/reference_cases/e4_pl_q1u_implementation_manifest.json",
    )
    ...
    implementation_review_path, implementation_review_value = _validate_review(
        implementation_review,
        expected_path="docs/reference_cases/e4_pl_q1u_implementation_review.json",
        expected_schema="anysolver.s4.e4-pl-q1u-implementation-review-v1",
        expected_verdict="ACCEPT_Q1U_IMPLEMENTATION_FREEZE_NO_P0_P1",
        bound=True,
    )
    ...
    if exact_backend_test["node_ids"] != [
        "tests/test_e4_pl_q1u_exact_backend.py::test_q1u_exact_backend_toy_cancellation_nested_radicals_inverse_rank_sign_and_serialization"
    ]:
    ...
    plan_review_path, plan_review_value = _validate_review(
        reviews["plan"],
        expected_path="docs/reference_cases/e4_pl_q1u_plan_review.json",
        expected_schema="anysolver.s4.e4-pl-q1u-plan-review-v1",
        expected_verdict="ACCEPT_Q1U_PREREGISTRATION_NO_P0_P1",
        bound=True,
    )
    _validate_reviewed_inputs(
        plan_review_value,
    ...
    terminal_path = _bound_path(
        ...
        expected_path="docs/reference_cases/e4_pl_q1u_terminal_table.json",
    )
...
def _validate_external_environment_root(
    ...
    record_path = ROOT / "docs/reference_cases/e4_pl_q1u_environment.json"
    ...
    if (
        record.get("schema") != "e4_pl_q1u_environment_record_v1"
...
def _validate_execution_commit_chain(
    ...
    expected_contract = (ROOT / "docs/reference_cases/e4_pl_q1u_execution_contract.json").resolve()
...
def _validate_execution_authority(
    ...
    contract_review_path, contract_review_value = _validate_review(
        ...
        expected_path="docs/reference_cases/e4_pl_q1u_contract_review.json",
        expected_schema="anysolver.s4.e4-pl-q1u-contract-review-v1",
        expected_verdict="ACCEPT_Q1U_EXECUTION_CONTRACT_NO_P0_P1",
        bound=False,
    )
    _validate_reviewed_inputs(
        ...
        [
            ...
            if path != "docs/reference_cases/e4_pl_q1u_contract_review.json"
        ],
    )
    if (
...
def execute(
    ...
    *,
    ...
    if precisions != (256, 512, 1024):
        raise PermissionError("precision sequence differs from frozen Q1U authority")
    ...
    if frame_failure:
        terminal = "NO_GO_E4_PL_Q1U_PATCH_OR_COVARIANCE"
    elif algebra_failure:
        terminal = "NO_GO_E4_PL_Q1U_LOCAL_ALGEBRA"
    elif patch_failure:
        terminal = "NO_GO_E4_PL_Q1U_PATCH_OR_COVARIANCE"
    elif unresolved:
        terminal = "UNCLASSIFIED_E4_PL_Q1U_LOCAL_PLANAR_IDENTITY"
    else:
        terminal = "PROVISIONAL_GO_E4_PL_Q1U_Q1B_PLAN"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.static_transcription or args.toy_exact_backend:
        ...
        try:
            ...
            sys.stderr.write(f"BLOCKED_E4_PL_Q1U_CONTRACT_OR_NONDETERMINISM: {exc}\n")
            return 2
