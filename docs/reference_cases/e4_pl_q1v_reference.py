"""Independent exact reference for the E4-PL-Q1V local qualification.
...
PLAN_COMMIT = "7a33044aee429557d770b914130df47105d6bec9"
PLAN_TREE = "7ae5c3278aed1e5937d90c308db6f570e424b1f7"
PLAN_SUBJECT = "docs: preregister E4 PL Q1V local completion"
IMPLEMENTATION_SUBJECT = "docs: freeze E4 PL Q1V commissioned exact implementations"
EXECUTION_SUBJECT = "docs: authorize E4 PL Q1V scientific execution"
EXECUTION_TOKEN = "AUTHORIZE_E4_PL_Q1V_SCIENTIFIC_EXECUTION"
PAYLOAD_SCHEMA = "e4_pl_q1v_common_certificate_payload_v1"
OUTPUT_SCHEMA = "anysolver.s4.e4-pl-q1v-reference-raw-v1"
...
_COMMISSIONING_CAPABILITY = object()
...
class Field:
    ...
    def with_sqrt(self, value: "Alg") -> tuple["Field", "Alg"]:
        ...
        if len(self.radicands) >= 5:
            raise ValueError("frozen Q1V exact field exceeds formal degree 32")
...
class Alg:
    ...
    def __mul__(self, other: object) -> "Alg":
        ...
        if not self.field.radicands:
            return Alg(self.field, (self.coeff[0] * rhs.coeff[0],))
        half = self.field.dimension // 2
        subfield = self.field.subfield()
        a = Alg(subfield, self.coeff[:half])
        b = Alg(subfield, self.coeff[half:])
        c = Alg(subfield, rhs.coeff[:half])
        d = Alg(subfield, rhs.coeff[half:])
        radicand = Alg(subfield, self.field.radicands[-1])
        real = a * c + (b * d) * radicand
        radical = a * d + b * c
        return Alg(self.field, real.coeff + radical.coeff)
...
def _contracts() -> dict[str, object]:
    ...
    manifest = result["e4_pl_q1v_inheritance_manifest"]
    if not isinstance(manifest, dict):
        raise ValueError("Q1V inheritance manifest must be an object")
    direct_rows = manifest["inputs"]  # type: ignore[index]
    if len(direct_rows) != 117 or len({str(row["path"]) for row in direct_rows}) != 117:
        raise ValueError("Q1V inheritance manifest must bind exactly 117 distinct paths")
...
def _equation7_frame(
    ...
    plus_squared = _dot3(plus, plus)
    diagonal_cross = _cross3(d1, d2)
    cross_norm = field.sqrt(_dot3(diagonal_cross, diagonal_cross))
    e_d1 = tuple(field.rational(geometry.nodes[2][i] - geometry.nodes[0][i]) for i in range(3))
    e_d2 = tuple(field.rational(geometry.nodes[1][i] - geometry.nodes[3][i]) for i in range(3))
    e_g1 = field.sqrt(_dot3(e_d1, e_d1))
    e_g2 = field.sqrt(_dot3(e_d2, e_d2))
    ...
    e_g3 = field.sqrt(_dot3(e_plus, e_plus))
    e_cross = _cross3(e_d1, e_d2)
    e_g4 = field.sqrt(_dot3(e_cross, e_cross))
    e_diagonal_complement = 2 * e_g4 / (e_g1 * e_g2 * e_g3)
    if e_g3 * e_g3 == plus_squared:
        t1_norm = e_g3
    elif e_diagonal_complement * e_diagonal_complement == plus_squared:
        t1_norm = e_diagonal_complement
    else:
        raise ValueError("numbered equation-7 diagonal normalization absent from frozen E field")
...
def static_transcription() -> dict[str, object]:
    ...
    if len(STATIC_OBLIGATION_SYMBOLS) != 25:
        raise AssertionError("reference implementation must preserve all 25 frozen mechanics obligations")
    ...
    if any(offset < 0 for offset in offsets) or offsets != sorted(offsets):
        raise AssertionError("Q1V g1..g5 generator schedule is absent or reordered")
    ...
    return {
        "schema": "anysolver.s4.e4-pl-q1v-reference-static-transcription-v1",
...
def toy_exact_backend_certificate() -> dict[str, object]:
    ...
    payload = {
        "schema": "anysolver.s4.e4-pl-q1v-reference-toy-exact-backend-v1",
        "candidate_id": CANDIDATE_ID,
        "implementation_id": IMPLEMENTATION_ID,
        "domain_equalities": {
...
Q1U_CLOSEOUT_COMMIT = "7d4ac30b4d50a1ee62edefbe5fb0198b47276360"
...
def _stage_paths(stage: str) -> list[str]:
    allowed = load_json(ROOT / "docs/reference_cases/e4_pl_q1v_allowed_extent.json")
...
def _validate_execution_contract(path: pathlib.Path, expected_sha256: str) -> dict[str, object]:
    expected_path = (ROOT / "docs/reference_cases/e4_pl_q1v_execution_contract.json").resolve()
    path = path.resolve()
    ...
    authority_contract = load_json(ROOT / "docs/reference_cases/e4_pl_q1v_authority_contract.json")
    ...
    environment_path = _bound_path(
        ...
        expected_path="docs/reference_cases/e4_pl_q1v_environment.json",
    )
    environment_record = load_json(environment_path)
    if environment != {
        "record_path": "docs/reference_cases/e4_pl_q1v_environment.json",
    ...
    inheritance_manifest = load_json(ROOT / "docs/reference_cases/e4_pl_q1v_inheritance_manifest.json")
    ...
    _bound_path(
        ...
        expected_path="docs/reference_cases/e4_pl_q1v_scientific_test_runner.py",
    )
    ...
    manifest_path = _bound_path(
        ...
        expected_path="docs/reference_cases/e4_pl_q1v_implementation_manifest.json",
    )
    ...
    implementation_review_path, implementation_review_value = _validate_review(
        implementation_review,
        expected_path="docs/reference_cases/e4_pl_q1v_implementation_review.json",
        expected_schema="anysolver.s4.e4-pl-q1v-implementation-review-v1",
        expected_verdict="ACCEPT_Q1V_IMPLEMENTATION_FREEZE_NO_P0_P1",
        bound=True,
    )
    ...
    if exact_backend_test["node_ids"] != [
        "tests/test_e4_pl_q1v_exact_backend.py::test_q1v_exact_backend_toy_cancellation_nested_radicals_inverse_rank_sign_and_serialization"
    ]:
    ...
    plan_review_path, plan_review_value = _validate_review(
        reviews["plan"],
        expected_path="docs/reference_cases/e4_pl_q1v_plan_review.json",
        expected_schema="anysolver.s4.e4-pl-q1v-plan-review-v1",
        expected_verdict="ACCEPT_Q1V_PREREGISTRATION_NO_P0_P1",
        bound=True,
    )
    _validate_reviewed_inputs(
        plan_review_value,
    ...
    terminal_path = _bound_path(
        ...
        expected_path="docs/reference_cases/e4_pl_q1v_terminal_table.json",
    )
...
def _validate_external_environment_root(
    ...
    record_path = ROOT / "docs/reference_cases/e4_pl_q1v_environment.json"
    ...
    if (
        record.get("schema") != "e4_pl_q1v_environment_record_v1"
...
def _validate_execution_commit_chain(
    ...
    if _git("show", "-s", "--format=%P", PLAN_COMMIT).split() != [Q1U_CLOSEOUT_COMMIT]:
    ...
    expected_contract = (ROOT / "docs/reference_cases/e4_pl_q1v_execution_contract.json").resolve()
...
def _validate_execution_authority(
    ...
    contract_review_path, contract_review_value = _validate_review(
        ...
        expected_path="docs/reference_cases/e4_pl_q1v_contract_review.json",
        expected_schema="anysolver.s4.e4-pl-q1v-contract-review-v1",
        expected_verdict="ACCEPT_Q1V_EXECUTION_CONTRACT_NO_P0_P1",
        bound=False,
    )
    _validate_reviewed_inputs(
        ...
        [
            ...
            if path != "docs/reference_cases/e4_pl_q1v_contract_review.json"
        ],
    )
    if (
...
def execute(
    ...
    *,
    ...
    if precisions != (256, 512, 1024):
        raise PermissionError("precision sequence differs from frozen Q1V authority")
    ...
    if frame_failure:
        terminal = "NO_GO_E4_PL_Q1V_PATCH_OR_COVARIANCE"
    elif algebra_failure:
        terminal = "NO_GO_E4_PL_Q1V_LOCAL_ALGEBRA"
    elif patch_failure:
        terminal = "NO_GO_E4_PL_Q1V_PATCH_OR_COVARIANCE"
    elif unresolved:
        terminal = "UNCLASSIFIED_E4_PL_Q1V_LOCAL_PLANAR_IDENTITY"
    else:
        terminal = "PROVISIONAL_GO_E4_PL_Q1V_Q1B_PLAN"
...
def commissioning_record(*, capability: object | None = None) -> dict[str, object]:
    """Construct the registered topology without producing scientific evidence."""

    if capability is not _COMMISSIONING_CAPABILITY:
        raise PermissionError("commissioning requires the guarded commissioning capability")
    contracts = _contracts()
    operations = _operations(contracts["e4_pl_q1r_frame_contract"])
    geometries = _geometries(contracts["e4_pl_q1r_geometry_contract"])
    ...
    for geometry in geometries:
        for operation in operations:
            ...
            t5, qd, p5, pd, local_to_global = _frame_embeddings(field, frame)
            centre_jacobian, centre_det, r_bar, s_bar = _centre_geometry_terms(field, coords)
            centre_taylor = _drill_taylor_rows(field, coords)
            source_sigma_shape: list[int] | None = None
            source_epsilon_shape: list[int] | None = None
            compatible_shape: list[int] | None = None
            case_id = f"{geometry.id}::{operation.id}"
            current_station_ids: list[str] = []
            for station_id, (r, s) in zip(GAUSS_IDS, _gauss(field)):
                _, dr, ds = _shape(field, r, s)
                jacobian, determinant = _jacobian(coords, dr, ds)
                dx, dy = _physical_gradients(jacobian, determinant, dr, ds)
                bm = _kinematic_membrane(field, dx, dy)
                bb = _kinematic_bending(field, dx, dy)
                bs = _kinematic_mitc_shear(field, coords, r, s, jacobian, determinant)
                compatible = _kinematic_source_operator(bm, bb, bs)
                n_sigma, n_epsilon = _source_mixed_fields(
                    field,
                    r,
                    s,
                ...
                source_sigma_shape = [len(n_sigma), len(n_sigma[0])]
                source_epsilon_shape = [len(n_epsilon), len(n_epsilon[0])]
                compatible_shape = [len(compatible), len(compatible[0])]
                full_station_id = f"{case_id}::{station_id}"
                current_station_ids.append(full_station_id)
                station_ids.append(full_station_id)
            centre_id = f"{case_id}::CENTRE"
            centre_ids.append(centre_id)
            dimensions = {
                "centre_taylor": [len(centre_taylor), len(centre_taylor[0])],
                "compatible": compatible_shape,
                "frame": [len(frame), len(frame[0])],
                "gauss_stations": len(current_station_ids),
                "independent_strain": source_epsilon_shape,
                "independent_stress": source_sigma_shape,
                "load": 24,
                "qd": [len(qd), len(qd[0])],
                "reaction": [24, 20],
                "support": [20, 24],
    ...
    construction = {
        ...
        "construction_completed": len(case_records) == 56,
        "dimensions": {
            "case_records": len(case_records),
            "centre_ids": len(centre_ids),
            "geometry_groups": len(geometries),
            "station_ids": len(station_ids),
        },
        "exception_status": "NONE",
        "schema_valid": len(case_records) == 56 and len(station_ids) == 224 and len(centre_ids) == 56,
        "station_ids": station_ids,
    }
    deterministic_payload = {
        "case_records": construction["case_records"],
        "centre_ids": construction["centre_ids"],
        "dimensions": construction["dimensions"],
        "station_ids": construction["station_ids"],
    }
    return {
        "candidate_id": CANDIDATE_ID,
        **construction,
        "determinism_payload_sha256": sha256_bytes(canonical_bytes(deterministic_payload)),
        "implementation_id": IMPLEMENTATION_ID,
...
def _parser() -> argparse.ArgumentParser:
    ...
    group.add_argument("--commission", action="store_true")
    group.add_argument("--authority-check-only", action="store_true")
    ...
    parser.add_argument("--commissioning-contract", type=pathlib.Path)
    parser.add_argument("--commissioning-contract-sha256")
    parser.add_argument("--environment-root", type=pathlib.Path)
    parser.add_argument("--environment-record", type=pathlib.Path)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.static_transcription or args.toy_exact_backend:
        ...
        if any(value is not None for value in prohibited.values()):
            raise SystemExit("commissioning accepts no registered-execution authority arguments")
        required = {
            "--commissioning-contract": args.commissioning_contract,
            "--commissioning-contract-sha256": args.commissioning_contract_sha256,
            "--environment-root": args.environment_root,
            "--environment-record": args.environment_record,
        ...
        if missing:
            raise SystemExit("commissioning requires: " + ", ".join(missing))
        try:
            if args.runner_id != "REFERENCE_COMMISSIONING_RUNNER":
                raise AuthorityGuardError(
                    "reference commissioning requires REFERENCE_COMMISSIONING_RUNNER"
                )
            validate_commissioning_authority(
                repository_root=ROOT,
                runner_id=args.runner_id,
                commissioning_contract_path=args.commissioning_contract,
                commissioning_contract_sha256=args.commissioning_contract_sha256,
            ...
            sys.stderr.write(f"BLOCKED_E4_PL_Q1V_IMPLEMENTATION_IDENTITY: {exc}\n")
            return 2
        raw = canonical_bytes(commissioning_record(capability=_COMMISSIONING_CAPABILITY))
    else:
        required = {
            "--authority-record": args.authority_record,
            "--authority-sha256": args.authority_sha256,
        ...
        if args.authority_check_only and args.output is not None:
            raise SystemExit("authority-check-only accepts no output path")
        try:
            if args.runner_id != "REFERENCE_RUNNER":
                raise AuthorityGuardError("reference executable requires REFERENCE_RUNNER")
            ...
            sys.stderr.write(f"BLOCKED_E4_PL_Q1V_CONTRACT_OR_NONDETERMINISM: {exc}\n")
            return 2
        if args.authority_check_only:
            raw = authority_check_bytes(
                {
                    "mode": "AUTHORITY_CHECK_ONLY",
                    "runner_id": "REFERENCE_RUNNER",
                    "schema": "anysolver.s4.e4-pl-q1v-authority-check-v1",
                    "status": "PASS",
                }
            )
        else:
            ...
            raw = canonical_bytes(
                execute(
                    ...
                    authority_sha256=args.authority_sha256.upper(),
