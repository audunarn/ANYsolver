"""Independent checker for canonical V5L Stage 4B worker proofs."""
...
def load(path: Path) -> dict[str, Any]:
    raw = path.read_bytes()
    value = json.loads(raw, object_pairs_hook=_pairs, parse_constant=lambda token: (_ for _ in ()).throw(CheckerError(f"nonfinite token: {token}")))
    if not isinstance(value, dict) or raw != canonical_bytes(value):
        raise CheckerError("V5L proof is noncanonical")
    return value
...
def verify(proof: dict[str, Any]) -> dict[str, Any]:
    if proof.get("schema") != SCHEMA or proof.get("activation_authorized") is not False:
        raise CheckerError("V5L proof identity changed")
    ...
    if worker_id.startswith("MODAL_"):
        ...
        if payload.get("frequency_gate_passed") is not (error <= 0.02) or payload.get("mac_gate_passed") is not (mac >= 0.95):
            raise CheckerError("V5L modal gate differs")
        ...
        for row in records:
            ...
            if float.fromhex(row["minimum_subspace_mac_hex"]) != score:
                raise CheckerError("V5L subspace score differs")
            scores.append(score)
        passed = max(errors) <= 0.03 and min(scores) >= 0.95
        if payload.get("factor_gate_passed") is not (max(errors) <= 0.03) or payload.get("mac_gate_passed") is not (min(scores) >= 0.95):
            raise CheckerError("V5L buckling gate differs")
    elif worker_id.startswith("PERFORMANCE_"):
        passed = payload == {"fraction_percent":int(worker_id.rsplit("_",1)[1]),"measurement_complete":True}
    else:
        raise CheckerError("unknown V5L worker")
    expected = PASS if passed else FAIL
    if proof.get("gate_status") != expected:
        raise CheckerError("V5L worker status differs")
    return {"accepted":True,"gate_status":expected,"proof_sha256":hashlib.sha256(canonical_bytes(proof)).hexdigest().upper(),"schema":"anysolver.e4-pl-s3-v5l-checker-v1","worker_id":worker_id}


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--verify-v5l-proof", action="store_true", required=True)
