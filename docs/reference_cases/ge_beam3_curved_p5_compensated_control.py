"""Explicit compensated-coordinate displacement control; author research only.

The old controller remains unchanged. Low parts enter every evaluation, trial,
failure snapshot and atomic commit/replay. No automatic retry or qualification.
"""
...
from docs.reference_cases.ge_beam3_curved_p5_assembly_history_probe import (
    AssemblyPathError, AssemblyTransactionError, _Budget,
)
...
from docs.reference_cases.ge_beam3_curved_p5_compensated_assembly import (
    CompensatedAssemblyHistoryProbe, CompensatedTrial, COORDINATES, move_pairs,
)
from docs.reference_cases.ge_beam3_curved_p5_displacement_control_probe import (
    DisplacementControlError, ControlObservationError, ControlCheckpoint, DisplacementTrial,
)


class CompensatedDisplacementControlledAssemblyProbe:
    def __init__(self,model,*,node,component=1):
        if type(model) is not CompensatedAssemblyHistoryProbe or model._pending is not None:
    ...
    def trial(self,target,*,max_iterations=16,max_mixed_evaluations=None,observer=None,failure_observer=None):
        ...
        x,low,u=state.positions.copy(),state.position_low.copy(),state.rotations.copy();checkpoints=[]
        observations=0
        ...
        def evaluate(a,b,low_parts):
            nonlocal last_evaluation
            response=staged._solve_all(a,b,origins,budget,position_low=low_parts)
            if failure_observer is not None: last_evaluation=(a,b,low_parts,response,budget.count)
            return response
        ...
        def move(a,b,low_parts,delta):
            ...
            made,made_low=move_pairs(a,low_parts,d[:,:3]);made[self._node,self._component]=target
            made_low[self._node,self._component]=0.
            return made,np.array([rotation(v[3:])@old for v,old in zip(d,b)]),made_low
        try:
            emit('INITIALIZATION',target=float(target),origin_epoch=state.epoch)
            response=evaluate(x,u,low)
            change=math.fsum((float(target),-float(x[self._node,self._component]),-float(low[self._node,self._component])))
            if change:
                ...
                x,u,low=move(x,u,low,delta);response=evaluate(x,u,low)
            for iteration in range(max_iterations+1):
                ...
                if norm<=1e-11:
                    if x[self._node,self._component]!=target or low[self._node,self._component]!=0.: raise DisplacementControlError('target not satisfied')
                    ...
                    assembly=CompensatedTrial(state.epoch,_readonly(x),_readonly(u),_readonly(forces),deepcopy(origins),
                ...
                for backtrack in range(10):
                    try:
                        a,b,candidate_low=move(x,u,low,delta*.5**backtrack);candidate=evaluate(a,b,candidate_low)
                    ...
                    if observer is not None:
                        emit('CANDIDATE',iteration,backtrack,residual=candidate_norm,accepted=candidate_norm<norm,
                             ...
                             position_low_change_max=float(np.max(np.abs(candidate_low-low))),
                    ...
                    if candidate_norm<norm:
                        x,u,low,response=a,b,candidate_low,candidate;break
            ...
            if failure_observer is not None and last_evaluation is not None:
                a,b,low_parts,response,evaluated_count=last_evaluation
                ...
                snapshot={'schema':'GE_BEAM3_P5_COMPENSATED_FAILED_LAST_EVALUATION_V1',
                    'disposition':'UNCOMMITTED_DIAGNOSTIC_ONLY','origin_epoch':state.epoch,
                    'target':float(target),'positions':a,'position_low':low_parts,'coordinate_schema':COORDINATES,
                          'rotations':b,'forces':forces,
    ...
    def commit(self,trial):
        ...
        if (trial.control_dof!=self._control or trial.assembly.positions[self._node,self._component]!=trial.target or
                trial.assembly.position_low[self._node,self._component]!=0. or
                digest(trial.assembly)!=digest(staged._pending)):
            raise DisplacementControlError('compensated control/staged trial mismatch')
