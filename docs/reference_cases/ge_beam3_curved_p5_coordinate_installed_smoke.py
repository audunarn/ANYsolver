...
def main():
    ...
    from anysolver._ge_beam3_p5_coordinates.core import canonical, sha
    from anysolver._ge_beam3_p5_coordinates.reference_modal import solve as modal
    from anysolver._ge_beam3_p5_coordinates.committed_modal import solve_elastic_modes
    ...
    def problem(yield_force):
        model = FEModel('p5-installed-candidate')
        nodes = np.array([[-1.,0.,0.],[0.,.4,0.],[1.,0.,0.]])
        frames = []
        for xi in (-1.,0.,1.):
    ...
    record = dict(schema='GE_BEAM3_P5_COORDINATE_INSTALLED_DEVELOPMENT_CHECK_V2',production_qualified=False,
