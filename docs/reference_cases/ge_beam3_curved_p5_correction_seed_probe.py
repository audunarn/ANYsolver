"""Successor elastic seed solver with correction-based line-search merit.

FAILED DEVELOPMENT EXPERIMENT: the two-element postcritical seed still reached
the line-search bound. This module is not a selected or qualified solver.
...
def correction_norm(vector, free, length):
    """Euclidean translational/span and spatial-rotation correction norm."""
    weights = np.where(np.asarray(free)%6<3,1/length,1.)
    return float(np.linalg.norm(np.asarray(vector)*weights))
        ...
        for iteration in range(max_iterations+1):
            ...
            for backtrack in range(10):
                ...
                try:
                    ...
                    continue
                simplified = np.linalg.solve(matrix,(candidate.residual-external)[free])
