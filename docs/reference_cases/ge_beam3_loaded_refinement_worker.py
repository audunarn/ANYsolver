...
import sys
from docs.reference_cases import ge_beam3_loaded_refinement_protocol as p
...
def run(path,digest,output):
    ...
    n=value['macros'];stage=value['stage'];programme=case.program(n,p.DROP)
    checkpoint=packet=science=None
    print(dict(stage='matched-drop-initialized',macros=n,worker=stage),flush=True)
    if stage<=4:
        ...
        if max(*row['metrics'],row['correction'],abs(row['control_value']-p.DROP))>1e-11 or row['parameter']!=made.parameter:
            raise ValueError('controlled equilibrium binding')
        publish(output/'science.json',dict(schema='GE_BEAM3_MATCHED_DROP_LOADED_REFINEMENT_V1',revision=value['revision'],macros=n,
            drop=p.DROP,load=made.parameter,negative_counts=counts,families=audits,geometry=geometry,
    ...
    publish(output/'ready.json',dict(schema='GE_BEAM3_MATCHED_DROP_READY_V1',revision=value['revision'],macros=n,stage=stage,
        request_sha256=digest,checkpoint=checkpoint,packet=packet,science=science,production_qualified=False))
    print(dict(stage='matched-drop-complete',macros=n,worker=stage),flush=True)
