"""Same-author Decimal audit of unexpanded supplied binary64 factor chains.
...
def symmetric_eigensystem(a,digits,checkpoint):
    a=[row[:] for row in a]; n=len(a); threshold=D(10)**(-(digits-20))
    vectors=[[D(int(i==j)) for j in range(n)] for i in range(n)]
    for sweep in range(100):
        checkpoint()
        maximum=max(abs(a[i][j]) for i in range(n) for j in range(i+1,n))
        if maximum<=threshold:
            order=sorted(range(n),key=lambda i:a[i][i])
            return [a[i][i] for i in order],[[row[i] for i in order] for row in vectors]
        for i in range(n):
            for j in range(i+1,n):
                ...
                for row in vectors:
                    x,y=row[i],row[j]; row[i]=c*x-s*y; row[j]=s*x+c*y
...
def factor_chain_modes(elastic,right,kinetic,free,algebraic,*,digits=80):
    ...
    n=len(kinetic[0]); link=len(right)
    if (not 1<=n<=48 or not 1<=link<=48 or any(len(row)!=link for row in elastic)
            or any(len(row)!=n for row in right+kinetic)):
        raise ValueError('bounded audit chain columns')
    if any(type(x) is not float for row in elastic+right+kinetic for x in row):
        raise ValueError('binary64 float chain data required')
    ...
    with localcontext() as context:
        context.prec=digits
        left=[[D.from_float(x) for x in row] for row in elastic]
        right=[[D.from_float(x) for x in row] for row in right]
        if any(not x.is_finite() for row in left+right for x in row): raise ValueError('finite chain factors')
        ...
        values,vectors=symmetric_eigensystem(normalized,digits,checkpoint)
        physical_modes=triangular_solve(transpose(lm),vectors,False)
        modes=[[D(0)]*len(physical) for _ in range(n)]
        for i,row in zip(physical,physical_modes): modes[i]=row
        if algebraic:
            trace_modes=product(eliminate,physical_modes)
            for i,row in zip(algebraic,trace_modes): modes[i]=[-x for x in row]
        return dict(eigenvalues=tuple(str(x) for x in values),
                    full_modes=tuple(tuple(str(x) for x in row) for row in modes),
                    digits=digits,certified_intervals=False)
