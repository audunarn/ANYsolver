"""Saved-factor physical pencil; Decimal assembly/trace elimination, no mechanics."""
...
def matrix(value,width):
    if type(value) is not list or not 1<=len(value)<=8192 or any(type(row) is not list or len(row)!=width
...
def assemble(packet,digits,check):
    ...
    if not 2<=n<=512 or not 1<=link<=512:raise ValueError('bounded saved factor dimensions')
