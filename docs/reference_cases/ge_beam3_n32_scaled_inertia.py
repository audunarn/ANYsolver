...
from docs.reference_cases.ge_beam3_n32_front_inertia import inertia as unscaled


def inertia(matrix, checkpoint=lambda:None):
    n=len(matrix)
    if not 1<=n<=640 or any(len(row)!=n for row in matrix):raise ValueError('bounded square matrix')
    ...
    return dict(**result,positive_diagonal_congruence=True,
