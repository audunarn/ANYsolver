#!/usr/bin/env python3
"""Mechanics-free guarded launcher for the Q1T scientific inventory.
...
AUTHORIZATION = "AUTHORIZE_E4_PL_Q1T_SCIENTIFIC_EXECUTION"
...
RESULT_SCHEMA = "anysolver.s4.e4-pl-q1t-scientific-test-result-v1"
BLOCKED_TERMINAL = "BLOCKED_E4_PL_Q1T_CONTRACT_OR_NONDETERMINISM"
...
REVIEW_PATHS = {
    "plan": "docs/reference_cases/e4_pl_q1t_plan_review.json",
    "implementation": "docs/reference_cases/e4_pl_q1t_implementation_review.json",
    "contract": "docs/reference_cases/e4_pl_q1t_contract_review.json",
}
...
NODE_IDS = [
    "tests/test_e4_pl_q1t_frame_and_fields.py::test_q1t_all_56_numbered_frames_and_field_work",
    "tests/test_e4_pl_q1t_local_algebra.py::test_q1t_actual_38_field_condensation_rank_and_rigid_modes",
    "tests/test_e4_pl_q1t_recovery.py::test_q1t_all_224_station_recovery_and_numerical_separation",
    "tests/test_e4_pl_q1t_global_supports.py::test_q1t_global_transform_load_support_solution_and_reactions",
    "tests/test_e4_pl_q1t_terminal_and_agreement.py::test_q1t_evidence_terminal_and_cross_implementation_contract",
]
RUNNER_IDS = ["REFERENCE_RUNNER", "ORACLE_RUNNER", RUNNER_ID]


class AuthorityError(RuntimeError):
    """Raised before collection when Q1T execution authority is invalid."""
...
def _repo_root() -> Path:
    ...
    if completed.returncode:
        raise AuthorityError("runner is not in the Q1T Git worktree")
...
def _verify_inherited_rows(root: Path, inherited: Mapping[str, Any]) -> None:
    ...
    if int(inherited["count"]) != 49 or not isinstance(rows, list) or len(rows) != 49:
        raise AuthorityError("Q1T must bind exactly 49 inherited rows")
    seen: set[str] = set()
    for row in rows:
    ...
    manifest_path = root / "docs/reference_cases/e4_pl_q1t_inheritance_manifest.json"
...
def verify_authority(
    ...
    """Authenticate Q1T execution without importing or collecting mechanics."""
    ...
    if contract_resolved != committed_contract:
        raise AuthorityError("contract path is not the committed Q1T contract")
    ...
    environment_path = root / "docs/reference_cases/e4_pl_q1t_environment.json"
    ...
    for commit, parent in ((head, commit2), (commit2, commit1), (commit1, Q1S_CLOSEOUT)):
        if _run_git(root, "rev-list", "--parents", "-n", "1", commit).split() != [commit, parent]:
            raise AuthorityError("Q1T authority commits must have exactly one parent")
    ...
    if environment_spec.get("path") != "docs/reference_cases/e4_pl_q1t_environment.json":
    ...
    if agreement.get("cross_implementation") != "BYTE_IDENTICAL_CANONICAL_COMMON_PAYLOAD":
    ...
    if terminal.get("path") != "docs/reference_cases/e4_pl_q1t_terminal_table.json":
        raise AuthorityError("wrong terminal-table path")
    if terminal.get("schema") != "anysolver.s4.e4-pl-q1t-terminal-table-v1":
    ...
    if terminal.get("terminal_count") != 11:
        raise AuthorityError("wrong Q1T terminal count")
...
def _execute_pytest(
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="anysolver-q1t-scientific-"))
    worktrees = _worktree_roots(root)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        root, authority, _contract, environment_root = verify_authority(
        ...
        if args.authority_check_only:
            result: dict[str, Any] = {
                ...
                "environment_sha256": args.environment_sha256.upper(),
                "runner_id": RUNNER_ID,
                "schema": "anysolver.s4.e4-pl-q1t-runner-authority-check-v1",
                "status": "AUTHORIZED_NO_MECHANICS_COLLECTION",
                "study_id": STUDY_ID,
            }
        else:
