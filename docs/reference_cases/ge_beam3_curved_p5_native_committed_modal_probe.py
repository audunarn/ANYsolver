"""Read-only accepted-origin P5 augmented tangent and elastic-interior modes.

Private development, not a registered capability. Plastic accepted-increment
Hessians are exposed as witnesses only, never silently called vibration moduli.
"""
...
from docs.reference_cases.ge_beam3_curved_p5_native_modal_probe import prepare as prepare_reference
from docs.reference_cases.ge_beam3_curved_p5_native_material_probe import sha
...
POLICY = 'P5_ACCEPTED_INCREMENT_AUGMENTED_HESSIAN_CURRENT_REST_MASS_V1'
...
def _operator(element, inner, inertia):
    ...
    mixed = CompensatedMixedBeamProbe(ref, core.section, order=core.order,
        position_low=np.zeros((3, 3)), origins=inner['origins'])
    evaluation = mixed.evaluate(inner['committed_positions'],
    ...
    kinetic = FiniteInertiaProbe(ref, inertia, order=core.order).evaluate(
        inner['committed_positions'], response.local_rotations, np.zeros(24), np.zeros(24))
    elastic = True
    ...
    return CommittedOperator(_owned(k), _owned(kinetic.mass), _owned(evaluation.residual[:24]),
