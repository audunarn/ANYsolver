"""Independent exact reference for the E4-PL-Q1S local qualification.
...
pre-authority command, ``--static-transcription``, validates committed input
syntax and the frozen numbered-frame tables without assembling a registered
case.  Scientific execution requires a committed caller-bound contract and
its SHA-256 on the command line.
...
IMPLEMENTATION_ID = "Q1S_REFERENCE_INDEPENDENT"
PLAN_COMMIT = "00d6a66c34712c8f3fd1e38113c83d0a03b2de43"
PLAN_TREE = "661bf7ce0c509adb3f8e0f0559974ce24171dda1"
PLAN_SUBJECT = "docs: preregister E4 PL Q1S implementation completion"
...
EXECUTION_TOKEN = "AUTHORIZE_E4_PL_Q1S_SCIENTIFIC_EXECUTION"
PAYLOAD_SCHEMA = "anysolver.s4.e4-pl-q1s-certificate-payload-v1"
OUTPUT_SCHEMA = "anysolver.s4.e4-pl-q1s-certificate-wrapper-v1"
...
def interval_of(value: Alg, bits: int) -> DyadicInterval:
    roots: list[DyadicInterval] = []

    def evaluate(coefficients: Sequence[Fraction]) -> DyadicInterval:
        result = DyadicInterval.exact(Fraction(0), bits)
        for mask, coeff in enumerate(coefficients):
            term = DyadicInterval.exact(coeff, bits)
    ...
    for radicand in value.field.radicands:
        roots.append(DyadicInterval.sqrt_interval(evaluate(radicand)))
    return evaluate(value.coeff)
...
def _contracts() -> dict[str, object]:
    ...
    manifest = result["e4_pl_q1s_inheritance_manifest"]
    if not isinstance(manifest, dict):
        raise ValueError("Q1S inheritance manifest must be an object")
    bound_rows = {
        str(row["path"]): row
        for group in ("inherited_q1r_inputs", "q1r_closeout_inputs", "inherited_e4_core_inputs")
        for row in manifest[group]  # type: ignore[index]
    }
    for relative in MECHANICS_INPUTS + tuple(INHERITED_CORE_INPUTS):
        ...
        if len(raw) != row["bytes"] or sha256_bytes(raw) != row["sha256"]:
            raise ValueError(f"inherited mechanics identity mismatch: {relative}")
    return result
...
def static_transcription() -> dict[str, object]:
    ...
    completeness = contracts["e4_pl_q1s_implementation_completeness"]
    required_ids = [str(row["id"]) for row in completeness["required_rows"]]
    if set(required_ids) != set(STATIC_OBLIGATION_SYMBOLS) or len(required_ids) != 25:
        raise AssertionError("static completeness obligation map is not the exact 25-row authority")
    ...
    return {
        "schema": "anysolver.s4.e4-pl-q1s-reference-static-transcription-v1",
...
Q1R_CLOSEOUT_COMMIT = "46231c56d4c7d24000421fc3ba0f4800239e64bd"
...
def _stage_paths(stage: str) -> list[str]:
    allowed = load_json(ROOT / "docs/reference_cases/e4_pl_q1s_allowed_extent.json")
...
def _validate_execution_contract(path: pathlib.Path, expected_sha256: str) -> dict[str, object]:
    expected_path = (ROOT / "docs/reference_cases/e4_pl_q1s_execution_contract.json").resolve()
    path = path.resolve()
    ...
    authority_contract = load_json(ROOT / "docs/reference_cases/e4_pl_q1s_authority_contract.json")
    ...
    inheritance_manifest = load_json(ROOT / "docs/reference_cases/e4_pl_q1s_inheritance_manifest.json")
    ...
    if inherited != {"count": 23, "rows": expected_inherited_rows}:
        raise PermissionError("23-row inheritance binding mismatch")
    ...
    implementations = {
        "reference": (
            ...
            "Q1S_REFERENCE_INDEPENDENT",
        ),
        "oracle": (
            "docs/reference_cases/e4_pl_q1s_oracle.py",
            "Q1S_ORACLE_INDEPENDENT",
        ),
    }
    ...
    implementation_review_path, implementation_review_value = _validate_review(
        implementation_review,
        expected_path="docs/reference_cases/e4_pl_q1s_implementation_review.json",
        expected_schema="anysolver.s4.e4-pl-q1s-implementation-review-v1",
        expected_verdict="ACCEPT_Q1S_IMPLEMENTATION_FREEZE_NO_P0_P1",
        bound=True,
    )
    ...
    expected_tests = _stage_paths("IMPLEMENTATION")[5:]
    ...
    plan_review_path, plan_review_value = _validate_review(
        reviews["plan"],
        expected_path="docs/reference_cases/e4_pl_q1s_plan_review.json",
        expected_schema="anysolver.s4.e4-pl-q1s-plan-review-v1",
        expected_verdict="ACCEPT_Q1S_PREREGISTRATION_NO_P0_P1",
        bound=True,
    )
    _validate_reviewed_inputs(
        plan_review_value,
    ...
    if contract_review != {
        "path": "docs/reference_cases/e4_pl_q1s_contract_review.json",
        "schema": "anysolver.s4.e4-pl-q1s-contract-review-v1",
        "verdict": "ACCEPT_Q1S_EXECUTION_CONTRACT_NO_P0_P1",
    ...
    expected_runner_rows = authority_contract["guard_runners"]  # type: ignore[index]
    ...
    if (
        ...
        or terminal["terminal_count"] != 12
        or len(terminal_value["terminals"]) != 12
    ):
...
def _validate_execution_commit_chain(
    ...
    if not _same_extent(_commit_paths(parents[0]), _stage_paths("IMPLEMENTATION")):
        raise PermissionError("Commit 2 does not have the exact ordered ten-path extent")
    ...
    if _git("show", "-s", "--format=%P", PLAN_COMMIT).split() != [Q1R_CLOSEOUT_COMMIT]:
        raise PermissionError("Commit 1 parent mismatch")
    if not _same_extent(_commit_paths(PLAN_COMMIT), _stage_paths("PLAN")):
        raise PermissionError("Commit 1 does not have the exact ordered eleven-path extent")

    expected_contract = (ROOT / "docs/reference_cases/e4_pl_q1s_execution_contract.json").resolve()
...
def _validate_execution_authority(
    ...
    expected_verdicts = {
        "plan": "ACCEPT_Q1S_PREREGISTRATION_NO_P0_P1",
        "implementation": "ACCEPT_Q1S_IMPLEMENTATION_FREEZE_NO_P0_P1",
        "contract": "ACCEPT_Q1S_EXECUTION_CONTRACT_NO_P0_P1",
    }
    if (
    ...
    contract_review_path, contract_review_value = _validate_review(
        ...
        expected_path="docs/reference_cases/e4_pl_q1s_contract_review.json",
        expected_schema="anysolver.s4.e4-pl-q1s-contract-review-v1",
        expected_verdict="ACCEPT_Q1S_EXECUTION_CONTRACT_NO_P0_P1",
        bound=False,
    )
    _validate_reviewed_inputs(
        ...
        [
            ...
            if path != "docs/reference_cases/e4_pl_q1s_contract_review.json"
        ],
    )
    if (
...
def execute(
    ...
    *,
    ...
    if precisions != (256, 512, 1024):
        raise PermissionError("precision sequence differs from frozen Q1S authority")
    ...
    if frame_failure:
        terminal = "NO_GO_E4_PL_Q1S_FRAME_IDENTITY"
    elif algebra_failure:
        terminal = "NO_GO_E4_PL_Q1S_LOCAL_ALGEBRA"
    elif patch_failure:
        terminal = "NO_GO_E4_PL_Q1S_PATCH_OR_COVARIANCE"
    elif unresolved:
        terminal = "UNCLASSIFIED_E4_PL_Q1S_LOCAL_PLANAR_IDENTITY"
    else:
        terminal = "PROVISIONAL_GO_E4_PL_Q1S_Q1B_PLAN"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.static_transcription:
        if any(
            ...
            raise SystemExit("static transcription accepts no execution or output arguments")
        raw = canonical_bytes(static_transcription())
    else:
        required = {
