...
def run(path,digest,output):
    ...
    print(dict(stage='fine-upper-initialized',macros=n,worker=stage),flush=True)
    if stage<=4:
        ...
        publish(output/'science.json',dict(schema='GE_BEAM3_FINE_UPPER_LOADED_V1',revision=value['revision'],macros=n,
    ...
    publish(output/'ready.json',dict(schema='GE_BEAM3_FINE_UPPER_READY_V1',revision=value['revision'],macros=n,stage=stage,
        request_sha256=digest,checkpoint=checkpoint,packet=packet,science=science,production_qualified=False))
    print(dict(stage='fine-upper-complete',macros=n,worker=stage),flush=True)
