"""Coupled hardening-section/partial-Legendre research, not a production law.

A declared single associative plastic flow direction exercises the nonlinear
section protocol without pretending to implement general fibre or J2 yielding.
The complete SPD 6x6 elastic matrix and all six strain/resultant components
are retained. Perfect plasticity and softening are not admitted in this probe.
"""
...
class DirectedHardeningSectionProbe:
...
def _digest(response):
    ...
    for key, value in vars(response).items():
        if isinstance(value, np.ndarray):
            value = value.tolist()
        elif isinstance(value, SectionHistory):
            value = vars(value)
        record[key] = value
...
class SectionTransactionError(RuntimeError):
    """Foreign, stale or altered research station response."""


class SectionStationProbe:
    """One station owns history; replay retains the accepted algorithmic origin.

    Elastic unloading from committed history is a *new trial*, not a replay
    of the preceding plastic increment. No serialization/restart API is
    supplied here; persisting the origin is a later required integration gate.
    """

    def __init__(self, section):
        if type(section) is not DirectedHardeningSectionProbe:
            raise ValueError("explicit directed-hardening research section required")
        self._section = section
        self._committed = SectionHistory()
        self._accepted = None
    ...
    @property
    def committed(self):
        return self._epoch, _history(self._committed)

    def trial(self, strain):
        self._pending = self._pending_digest = None
        response = self._section.strain_response(strain, self._committed)
        self._pending, self._pending_digest = response, _digest(response)
        return response

    def _owned(self, response):
        if response is None or response is not self._pending:
            raise SectionTransactionError("owned current station trial required")

    def discard(self, response):
        self._owned(response)
        self._pending = self._pending_digest = None

    def commit(self, response):
        self._owned(response)
        try:
            if _digest(response) != self._pending_digest:
                raise SectionTransactionError("altered station trial")
        except (ValueError, TypeError) as exc:
            raise SectionTransactionError("altered station trial") from exc
        # Recompute from the accepted origin so caller-owned arrays are not
        # retained in authoritative committed or replay data.
        accepted = self._section.strain_response(response.strain, self._committed)
        if _digest(accepted) != self._pending_digest:
            raise SectionTransactionError("station implementation changed during trial")
        self._committed = _history(accepted.history)
        self._accepted, self._accepted_digest = accepted, _digest(accepted)
        self._epoch += 1
        self._pending = self._pending_digest = None

    def replay(self):
        if self._accepted is None:
            raise SectionTransactionError("no accepted station increment to replay")
        response = self._section.strain_response(self._accepted.strain, self._accepted.origin)
        if _digest(response) != self._accepted_digest:
            raise SectionTransactionError("accepted station replay mismatch")
        return response
