#!/usr/bin/env python3
"""Independent SymPy exact oracle for the frozen E4-PL-Q1V study.

There are deliberately five disjoint entry paths:
...
* ``--commission`` constructs nonclassifying implementation evidence only;
* ``--authority-check-only`` completes the shared execution guard only;
* ``--execute`` completes that same guard, then (and only then) constructs
  registered geometry and mechanics.
...
WRAPPER_SCHEMA = "anysolver.s4.e4-pl-q1v-oracle-raw-v1"
COMMISSIONING_SCHEMA = "anysolver.s4.e4-pl-q1v-oracle-commissioning-v1"
COMMISSIONING_RECORD_KIND = "NONCLASSIFYING_IMPLEMENTATION_COMMISSIONING"
...
SUCCESS_TERMINAL = "PROVISIONAL_GO_E4_PL_Q1V_Q1B_PLAN"
LOCAL_NO_GO = "NO_GO_E4_PL_Q1V_LOCAL_ALGEBRA"
PATCH_NO_GO = "NO_GO_E4_PL_Q1V_PATCH_OR_COVARIANCE"
UNCLASSIFIED_TERMINAL = "UNCLASSIFIED_E4_PL_Q1V_LOCAL_PLANAR_IDENTITY"
...
# ---------------------------------------------------------------------------
# Nonclassifying implementation commissioning


def _commissioning_payload(sympy: Any) -> dict[str, Any]:
    """Construct frozen registered objects without evaluating scientific gates."""

    geometry_specs, operations, _, cases = _frozen_inputs()
    field_cache: dict[str, tuple[FieldContext, tuple[Expr, ...]]] = {}
    ...
    for geometry_id, node_text in geometry_specs:
        base = _numbered_geometry(geometry_id, node_text, operations[0], sympy, field_cache)
        for operation in operations:
            geometry = (
                base
                if operation.operation_id == "E"
            ...
            if not geometry.frame_exact or not _frame_identity(base, geometry):
                raise OracleError("commissioning frame construction did not complete exactly")
            t5, qd = _t5_qd(geometry.frame)
            if shape(t5) != (24, 20) or shape(qd) != (24, 4):
                raise OracleError("commissioning T5/QD dimensions changed")
            gauss = _gauss_points(geometry.field, geometry.roots[4])
            if len(gauss) != 4:
                raise OracleError("commissioning Gauss construction count changed")
            for r, s in gauss:
                _jacobian(geometry, r, s)
                nsigma, nepsilon = _independent_interpolations(geometry, r, s)
                compatible = _compatible_b(geometry, r, s)
                if shape(nsigma) != (8, 14) or shape(nepsilon) != (8, 21) or shape(compatible) != (8, 20):
                    raise OracleError("commissioning source-field dimensions changed")
            centre_taylor = _centre_taylor(geometry)
            residual = _residual_gamma(geometry)
            if shape(centre_taylor) != (3, 24) or len(residual) != 4:
                raise OracleError("commissioning centre construction dimensions changed")
            local_load = _load_vector(geometry.field, cases)
            global_load = matvec(t5, local_load)
            support = transpose(t5)
            reaction = transpose(support)
            support_system = zeros(geometry.field, 44, 44)
            for i in range(20):
                for j in range(24):
                    support_system[24 + i][j] = support[i][j]
                    support_system[j][24 + i] = reaction[j][i]
            if len(global_load) != 24 or shape(support) != (20, 24) or shape(reaction) != (24, 20):
                raise OracleError("commissioning support/reaction dimensions changed")
            if shape(support_system) != (44, 44):
                raise OracleError("commissioning support system dimensions changed")
            case_id = f"{geometry_id}::{operation.operation_id}"
            case_station_ids = [f"{case_id}::{label}" for label in GAUSS_LABELS]
            case_records.append(
                {
                    "case_id": case_id,
            ...
            centre_ids.append(f"{case_id}::CENTRE")
            station_ids.extend(case_station_ids)
    if len(case_records) != 56 or len(station_ids) != 224 or len(centre_ids) != 56:
        raise OracleError("commissioning coverage changed")
    ...
    result = {
        ...
        "centre_ids": centre_ids,
        "construction_completed": True,
        "determinism_payload_sha256": sha256(canonical_bytes(deterministic_content)),
        "dimensions": deterministic_content["dimensions"],
        "exception_status": "NONE",
        "implementation_id": IMPLEMENTATION_ID,
        "record_kind": COMMISSIONING_RECORD_KIND,
    ...
    expected_case_keys = {"case_id", "construction_completed", "dimensions", "exception_status", "schema_valid"}
    if set(result) != expected_keys or any(set(row) != expected_case_keys for row in case_records):
        raise OracleError("commissioning schema changed")
    return result


# ---------------------------------------------------------------------------
# Shared fail-closed execution guard boundary
...
def _write_exclusive_verified(output: Path, raw: bytes) -> None:
    try:
        ...
        if reopened != raw or sha256(reopened) != sha256(raw):
            raise OracleError("exclusive output write verification failed")
    except OSError as exc:
        raise OracleError("exclusive output write failed") from exc


def _forbidden_commissioning_tokens(value: Any) -> Iterator[str]:
    if isinstance(value, Mapping):
        for key, item in value.items():
            yield str(key).upper()
            yield from _forbidden_commissioning_tokens(item)
...
def _commission(inputs: CommissionInputs) -> dict[str, Any]:
    guard: CommissioningGuardEvidence = validate_commissioning_authority(
        repository_root=ROOT,
        runner_id=inputs.runner_id,
        commissioning_contract_path=inputs.commissioning_contract,
        commissioning_contract_sha256=inputs.commissioning_contract_sha256,
        environment_root=inputs.environment_root,
        environment_record_path=inputs.environment_record,
    ...
    sympy = _load_sympy()
    result = _commissioning_payload(sympy)
    forbidden = {str(token).upper() for token in guard.contract["forbidden_content"]}
    found = forbidden.intersection(_forbidden_commissioning_tokens(result))
    if found:
        raise OracleError(f"commissioning output contains forbidden token: {sorted(found)[0]}")
    raw = canonical_bytes(result)
    if strict_json(raw) != result:
        raise OracleError("commissioning canonical serialization failed")
    _write_exclusive_verified(guard.output_path, raw)
    return result


def _execute(inputs: ExecuteInputs) -> dict[str, Any]:
    guard: GuardEvidence = validate_execution_authority(
        repository_root=ROOT,
        runner_id=inputs.runner_id,
        authority_record_path=inputs.authority_record,
    ...
    raw_output = inputs.output.absolute()
    if any(path.is_symlink() for path in (raw_output, *raw_output.parents) if path.exists()):
        raise OracleError("output path must not traverse a symlink")
    output = raw_output.resolve(strict=False)
    repository_root = ROOT.resolve(strict=True)
    if output == repository_root or repository_root in output.parents or any(
        (parent / ".git").exists() for parent in output.parents
    ):
        raise OracleError("output must be outside every Git worktree")
    if output.exists() or output.is_symlink():
        raise OracleError("output must be an absent non-symlink path")
    try:
        output_parent = output.parent.resolve(strict=True)
    except OSError as exc:
        raise OracleError("output parent must exist") from exc
    if not output_parent.is_dir() or output_parent.is_symlink():
        raise OracleError("output parent must be an existing non-symlink directory")
    ...
    wrapper = {
        ...
        "execution_contract_sha256": inputs.contract_sha256.upper(),
        "execution_authority_sha256": inputs.authority_sha256.upper(),
        "exact_environment_sha256": inputs.environment_sha256.upper(),
        "implementation_diagnostics": diagnostics,
    }
    raw = canonical_bytes(wrapper)
    _write_exclusive_verified(output, raw)
    return wrapper
...
def _parser() -> argparse.ArgumentParser:
    ...
    modes.add_argument("--commission", action="store_true")
    ...
    parser.add_argument("--commissioning-contract", type=Path)
    ...
    try:
        ...
        if args.toy_exact_backend:
            ...
            if args.environment_root is not None:
                ...
                try:
                    ...
                    raise OracleError("toy environment root is invalid") from exc
        ...
        if args.commission:
            ...
            if any(value is not None for value in forbidden):
                raise OracleError("commissioning forbids execution-authority inputs")
            values = (
                args.commissioning_contract,
                args.commissioning_contract_sha256,
                args.environment_root,
            ...
            if any(value is None for value in values):
                raise OracleError("commissioning requires all commissioning authority inputs")
            if args.runner_id != "ORACLE_COMMISSIONING_RUNNER":
                raise OracleError("commissioning requires runner ID ORACLE_COMMISSIONING_RUNNER")
            _commission(
                CommissionInputs(
                    args.commissioning_contract,
                    args.commissioning_contract_sha256,
            ...
            return 0
        if args.commissioning_contract is not None or args.commissioning_contract_sha256 is not None:
            raise OracleError("execution authority modes forbid commissioning-contract inputs")
        values = (
            args.authority_record,
            args.authority_sha256,
            args.contract,
        ...
        if any(value is None for value in values):
            raise OracleError("authority modes require all caller authority inputs")
        if args.runner_id != "ORACLE_RUNNER":
            raise OracleError("oracle requires runner ID ORACLE_RUNNER")
        if args.authority_check_only:
            if args.output is not None:
                raise OracleError("authority-check-only forbids an output path")
            validate_execution_authority(
                repository_root=ROOT,
        ...
        if args.output is None:
            raise OracleError("execute requires an output path")
        _execute(
            ExecuteInputs(
                args.authority_record,
                args.authority_sha256,
        ...
        return 0
    except (AuthorityGuardError, OracleError) as exc:
        print(f"Q1V_ORACLE_FAIL_CLOSED: {exc}", file=sys.stderr)
        return 2
