"""One-use force-accurate two-state comparison; research diagnostics only."""
...
BASE = '6ef2a405365ae786c7d0022f660188996a51a8ea'
MODULE = 'docs.reference_cases.ge_beam3_curved_p5_force_accuracy32_diagnostic'
SCHEMA = 'GE_BEAM3_P5_FORCE_ACCURACY32_DIAGNOSTIC_V1'
...
ALLOWED = {'docs/reference_cases/ge_beam3_curved_p5_displacement_control_probe.py',
           'docs/reference_cases/ge_beam3_curved_p5_force_accuracy32_diagnostic.py',
           'tests/test_ge_beam3_curved_p5_failure_snapshot.py',
           'tests/test_ge_beam3_curved_p5_force_accuracy32_diagnostic.py',
           'docs/agent_plans/GE_BEAM3_CURVED_P5_FORCE_ACCURACY32_PLAN.md'}
CASE = {'elements': 32, 'extent': 'ARCH_ONSET32', 'targets': [.1, .095],
        ...
        'assembly_schema': 'GE_BEAM3_P5_ASSEMBLY_LOCAL_FORCE_ACCURACY_V1',
...
def authority(repo, commit):
    ...
    return {'commit': commit, 'tree': shared.git(repo, 'rev-parse', 'HEAD^{tree}'),
            ...
            'failed_inputs': failed_inputs(), 'observation_inputs': observation_inputs()}
...
def lease(repo, commit, output, *, manager=None):
    ...
    if any(p.name.endswith(request_id) and p.name != 'ge-beam3-force-accuracy32-'+request_id
...
def capture(*, count, observe, publish_raw):
    ...
    beam = ForceAccurateAssemblyHistoryProbe(beam._references,
...
def worker(repo, commit, output):
    ...
    folder = output/'force-accuracy32'
...
def _coordinate(repo, commit, output):
    frozen = authority(repo, commit);request_id, request_sha = lease(repo, commit, output)
    if output.exists() or output.is_symlink() or output.resolve().is_relative_to(repo.resolve()) or output.resolve().is_relative_to(FAILED_ROOT.resolve()) or output.resolve().is_relative_to(OBSERVATION_ROOT.resolve()):
        raise ValueError('fresh external observation output required')
    (MANAGER/'claims'/('ge-beam3-force-accuracy32-'+request_id)).mkdir();output.mkdir(parents=True, exist_ok=False)
    shared.publish(output/'inputs.json', {'authority': frozen, 'case': CASE, 'request_id': request_id, 'request_sha256': request_sha})
    folder = output/'force-accuracy32';folder.mkdir();process = None
    try:
        ...
        result = {'schema': SCHEMA, 'disposition': 'RESEARCH_FORCE_ACCURACY_COMPARISON_CAPTURED',
