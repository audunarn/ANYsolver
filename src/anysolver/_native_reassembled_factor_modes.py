...
from anysolver._dyadic_bilinear import reassemble_exact_binary64
...
def reassemble(f,g,b,mapping,checkpoint=lambda:None):
    return reassemble_exact_binary64(f,g,b,mapping,checkpoint)
...
class ReassembledFactorModes:
    ...
    policy: str='PRIVATE_COMPLETE_MAP_REASSEMBLED_SIGNED_MODES_V1'
    bilinear_arithmetic: str='EXACT_DYADIC_INPUT_CONGRUENCE_ROUNDED_ONCE'
    negative_eigenvalues_retained: bool=True
    certified_intervals: bool=False
    production_qualified: bool=False


def solve_reassembled_factor_modes(factor,geometric,kinetic,free,algebraic,*,bounds,num_modes=6,root_width=1e-10,cancellation_token=None):
    start=monotonic()
    ...
    strain=mm(f,mapping,checkpoint); material_action=mm(strain.T,mm(strain,c,checkpoint),checkpoint)
    ...
    original_h,original_m=reassemble_exact_binary64(f,g,b,modes,checkpoint)
    ...
    checkpoint('output')
    return ReassembledFactorModes(_owned(values),_owned(modes),_owned(intervals),error,backward,original_error)
