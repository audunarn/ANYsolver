"""Private signed modes at accepted native V4 elastic-interior states.
...
from ._native_signed_factor_modes import solve_signed_factor_modes
from ._ge_beam3_p5_loads.core import sha
from ._ge_beam3_p5_loads.state import LOAD_POLICY
...
class SignedLoadedModes:
    ...
    policy: str = 'GE_BEAM3_V4_SIGNED_FACTOR_LOADED_MODES_V1'
...
def solve_signed_loaded_modes(model, element_states, displacement, section_inertias,
    ...
    packet, guard = prepare(model, states, displacement, section_inertias,
    ...
    rows = []; geometric = np.zeros_like(packet.stiffness)
    ...
    result = solve_signed_factor_modes(factor, geometric, packet.mass, packet.free_dofs,
