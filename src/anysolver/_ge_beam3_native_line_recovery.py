...
import numpy as np
from ._ge_beam3_native_line_static_element import NativeLineFibreStaticElement,FORMULATION,SCHEMA
...
def recover_native_fields(element,mesh,state,*,expected_committed_total_u=None):
    if type(element) is not NativeLineFibreStaticElement: raise ValueError('exact native physical fibre recovery element')
