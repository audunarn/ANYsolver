"""Private conservative line controller with equilibrium-first force recovery.
...
from ._ge_beam3_equilibrium_schur_solver import EquilibriumSchurSolver as ResultantSchurSolver, POLICY as LINEAR_POLICY
from ._ge_beam3_p5_seeded.core import sha

PROGRAM='GE_BEAM3_CONSERVATIVE_LINE_EQUILIBRIUM_FORCE_RECOVERY_DEVELOPMENT_V1'
...
def solve_force_program(model, program, *, line_forces, checkpoint=None,
    ...
    started = monotonic(); cancellation_safe_point(cancellation_token, 'fibre-equilibrium-line.start')
    context = Context(model, program, line_forces=line_forces, max_coordinates=max_coordinates,
        check=lambda: cancellation_safe_point(cancellation_token, 'fibre-equilibrium-line.material'))
    ...
    def safe(stage, target, iteration=0, **data):
        ...
        if monotonic()-started > 120.: raise RuntimeError('fibre equilibrium line cooperative deadline')
    ...
    try:
        safe('fibre-equilibrium-line.initialized', accepted.completed_targets)
        for index in range(accepted.completed_targets, end):
            ...
            for iteration in range(program.max_iterations+1):
                safe('fibre-equilibrium-line.before_assembly', index+1, iteration)
                residual, hessian, metrics, _ = context.assemble(trial, parameter, origins)
                safe('fibre-equilibrium-line.iteration', index+1, iteration, metrics=metrics)
                if max(metrics) <= 1e-11:
                    ...
                    safe('fibre-equilibrium-line.before_commit', index+1, iteration)
                    accepted, records, capsule = proposed, next_records, staged
                    safe('fibre-equilibrium-line.committed', index+1, iteration)
                    break
                if iteration == program.max_iterations: raise RuntimeError('fibre equilibrium line Newton limit')
                safe('fibre-equilibrium-line.before_factorization', index+1, iteration)
                ...
                safe('fibre-equilibrium-line.factorized', index+1, iteration, linear_solver=factor.diagnostics())
                ...
                for cut in range(program.max_backtracks+1):
                    safe('fibre-equilibrium-line.before_trial', index+1, iteration)
                ...
                else: raise RuntimeError('fibre equilibrium line search limit')
