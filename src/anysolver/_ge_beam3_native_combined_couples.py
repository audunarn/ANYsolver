...
from ._ge_beam3_fibre_distributed_couples import POLICY as DISTRIBUTED_POLICY
POLICY='GE_BEAM3_NATIVE_COMBINED_SPATIAL_COUPLES_V1'
from ._ge_beam3_native_line_loading import LinePattern
from ._ge_beam3_native_distributed_program import model_identity,solve_distributed_model
...
_RUN=ContextVar('ge_beam3_native_combined_couples',default=None)
    ...
    def descriptor(self):
        ...
        if self.restart_sha256 is not None:
            value.update(restart_sha256=self.restart_sha256,moment_restart_authorized=True,
                restart_policy='GE_BEAM3_SUPPORTED_NATIVE_COMBINED_COUPLE_RESTART_V1')
        return value
...
def decode_active_restart(model,raw,*,expected_sha256):
    from ._ge_beam3_native_combined_restart import decode_checkpoint
    run=_RUN.get()
...
def solve_combined_static(model,nodal_moments,*,distributed_pattern=None,constant_distributed=None,constant_moments=None,
    ...
    try:
        ...
        result.info['native_combined_couples']=run.descriptor()
