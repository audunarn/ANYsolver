...
import numpy as np
from ._ge_beam3_native_fibre_static_element import NativeFibreStaticElement,FORMULATION,SCHEMA
...
def recover_native_fields(element,mesh,state,*,expected_committed_total_u=None):
    if type(element) is not NativeFibreStaticElement: raise ValueError('exact native physical fibre recovery element')
    ...
    return dict(formulation_id=FORMULATION,state_schema=SCHEMA,state_sha256=state['state_sha256'],element_id=element.element_id,
