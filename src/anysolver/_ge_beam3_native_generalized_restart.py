"""Typed, load-path-bound accepted chains for private native generalized statics.
...
from ._ge_beam3_native_generalized_element import NativeGeneralizedStaticElement, SCHEMA as STATE_SCHEMA
...
from ._ge_beam3_generalized_static_boundary import GeneralizedStaticTrial
from ._ge_beam3_p5_seeded.core import canonical, sha
from ._ge_beam3_p5_seeded.codec import _load, MAX_BYTES

SCHEMA = 'GE_BEAM3_SUPPORTED_NATIVE_GENERALIZED_STATIC_RESTART_V1'
...
class LoadPoint:
    ...
    def require(self, model):
        ...
        if not 0. <= self.parameter <= 1. or type(self.constant) is not DistributedPattern or type(self.proportional) is not DistributedPattern:
            raise ValueError('exact bounded native generalized load point')
...
def _pattern(value, model):
    ...
    if type(value['couples']) is not list or any(type(row) is not list for row in value['couples']):
        raise ValueError('native generalized restart couple rows')
    made=DistributedPattern(_line_pattern(value['line'],model),tuple(tuple(row) for row in value['couples']))
    made.require(model.mesh)
    if canonical(made)!=canonical(value):raise ValueError('native generalized restart pattern signature/round trip')
    return made
...
def _point(value, model):
    ...
    made.require(model)
    if canonical(made)!=canonical(value): raise ValueError('native generalized restart load point round trip')
    return made
...
def _history(value,element):
    ...
    if type(value['cell_identity']) is not str or type(value['stations']) is not list or len(value['stations'])!=2*element.operator.order:
        raise ValueError('native generalized station history extent')
    stations=[]
    for station in value['stations']:
        _keys(station,('section_identity','plastic','accumulated'))
        if type(station['section_identity']) is not str:raise ValueError('native generalized section identity')
        plastic=_array(station['plastic'],(6,2));accumulated=_array(station['accumulated'],(2,))
        stations.append(GeneralizedHistory(station['section_identity'],tuple(tuple(map(float,row)) for row in plastic),tuple(map(float,accumulated))))
    made=GeneralizedCellHistory(value['cell_identity'],tuple(stations))
    with localcontext() as context:
        context.prec=80;element.operator.cell._origins(made)
    if canonical(made)!=canonical(value):raise ValueError('native generalized history round trip')
    return made
...
def _state(value, element, model):
    ...
    if value['schema']!=STATE_SCHEMA: raise ValueError('native generalized restart state schema')
    made=dict(value)
    ...
    if type(r['iterations']) is not int or not 0<=r['iterations']<=24 or type(r['input_sha256']) is not str:
        raise ValueError('native generalized restart response metadata')
    made['response']=GeneralizedStaticTrial(_array(r['rotations'],(2,3,3)),_array(r['resultants'],(18,)),_array(r['residual'],(18,)),
    ...
    if canonical(made)!=canonical(value): raise ValueError('native generalized restart typed state round trip')
    return made


def capture_solver_state(model,element_id,state,*,exact_guard):
    element=model.mesh.elements.get(element_id)
    if type(element) is not NativeGeneralizedStaticElement: raise TypeError('exact native generalized restart element')
    from ._ge_beam3_native_generalized_program import require_active
    try:programme=require_active(model)
    except ValueError as exc:raise ValueError('native generalized restart requires an authenticated chain') from exc
    if programme.initial is None:
        raise ValueError('native generalized restart requires an authenticated chain')
    try: raw=canonical(state)
    finally: exact_guard(model,context='native generalized restart input observation')
    if raw!=canonical(programme.initial['states'].get(element_id)):
        raise ValueError('native generalized initial state differs from authenticated chain')
    made=_state(_load(raw.decode('ascii')),element,model)
    element._validate(model.mesh,made)
    programme.require(model)
    exact_guard(model,context='native generalized restart input validation')
    return made
...
def _model(model, *, retained=False):
    ...
    for index,node in enumerate(nodes):
        if tuple(model.mesh.dof_manager.get_node_dofs(node))!=tuple(range(6*index,6*index+6)):
            raise ValueError('native generalized restart node/DOF ordering')
    ...
    if info['slave_dofs'] or np.any(offset) or t.nnz!=len(free) or np.any(t.data!=1.) or len(free)==n:
        raise ValueError('native generalized restart homogeneous supported selection required')
...
def supported_solver_coordinates(model,transform,offset,displacements):
    ...
    if transform.shape!=expected.shape or (transform!=expected).nnz or np.any(offset):
        raise ValueError('native generalized restart exact supported map')
    u=np.asarray(displacements)
    if u.shape!=(n,) or u.dtype!=np.float64 or not np.isfinite(u).all() or np.any(u[list(fixed)]):
        raise ValueError('native generalized restart supported displacement')
    q=u[list(free)].copy()
    if not np.array_equal(transform@q,u): raise ValueError('native generalized restart coordinate reconstruction')
    return q,0.
...
def _wire(snapshot,elements):
    ...
    if type(snapshot['load_point']) is not LoadPoint or type(snapshot['states']) is not dict or set(snapshot['states'])!={i for i,_ in elements}:
        raise ValueError('complete typed native generalized restart snapshot')
...
def validate_chain(model,snapshots):
    ...
    if type(snapshots) is not tuple or not 1<=len(snapshots)<=65: raise ValueError('bounded complete native generalized chain')
    previous=None
    for index,snapshot in enumerate(snapshots):
        ...
        if u.shape!=(n,) or u.dtype!=np.float64 or not np.isfinite(u).all() or np.any(u[list(fixed)]):
            raise ValueError('native generalized restart displacement/support mismatch')
        if index==0 and (point.parameter!=0. or pattern.line.rows or pattern.couples or np.any(u)):
            raise ValueError('native generalized restart stress-free genesis required')
        net=np.zeros(n); shared={}
        for eid,e in elements:
            if monotonic()-started>60.: raise RuntimeError('native generalized restart validation deadline')
            ...
            if state['epoch']!=index or state['load_pattern']!=pattern:
                raise ValueError('native generalized restart epoch/effective-load mismatch')
            ...
            for local,node in enumerate(e.node_ids):
                ...
                if node in shared and not np.array_equal(shared[node],rotation): raise ValueError('native generalized shared rotation disagreement')
            ...
            if previous is not None:
                ...
                if state['previous_state_sha256']!=old['state_sha256'] or canonical(state['origins'])!=canonical(old['response'].history):
                    raise ValueError('native generalized restart predecessor/history mismatch')
                if not np.array_equal(state['seed_rotations'],old['response'].rotations) or not np.array_equal(state['seed_resultants'],old['response'].resultants):
                    raise ValueError('native generalized restart internal seed mismatch')
                ...
                if not np.array_equal(expected,state['committed_nodal_rotation_matrices']): raise ValueError('native generalized multiplicative rotation mismatch')
        ...
        if not np.isfinite(norm) or not np.isfinite(residual_norm):
            raise ValueError('native generalized restart finite norm range required')
        error=residual_norm/max(1.,norm)
        if not np.isfinite(error) or error>1e-11: raise ValueError('native generalized accepted snapshot not equilibrated')
        previous=snapshot
    if _model(model)[-1]!=identity: raise ValueError('native generalized restart model changed')
    return identity
...
def encode_checkpoint(model,snapshots):
    ...
    if len(observed)>MAX_BYTES:raise ValueError('native generalized restart byte limit')
    identity=validate_chain(model,snapshots)
    if canonical([_wire(s,elements) for s in snapshots])!=observed:raise ValueError('native generalized chain changed during validation')
    ...
    if len(raw)>MAX_BYTES: raise ValueError('native generalized restart byte limit')
    return raw
...
def decode_checkpoint(model,raw,*,expected_sha256):
    if type(raw) is not bytes or type(expected_sha256) is not str or sha256(raw).hexdigest()!=expected_sha256:
        raise ValueError('native generalized restart external SHA-256 mismatch')
    ...
    if value['schema']!=SCHEMA or value['production_qualified'] is not False or value['model_sha256']!=identity or value['checkpoint_sha256']!=sha(body):
        raise ValueError('native generalized restart model/schema/hash mismatch')
    if type(value['snapshots']) is not list or not 1<=len(value['snapshots'])<=65: raise ValueError('native generalized snapshot count')
    snapshots=[]
    for record in value['snapshots']:
        _keys(record,('load_point','displacements','states')); rows=record['states']
        if type(rows) is not list or len(rows)!=len(elements): raise ValueError('native generalized restart complete states')
        states={}
        for row,(eid,e) in zip(rows,elements):
            _keys(row,('element_id','state'))
            if type(row['element_id']) is not int or row['element_id']!=eid: raise ValueError('native generalized ordered element identity')
    ...
    if encode_checkpoint(model,snapshots)!=raw: raise ValueError('native generalized restart canonical typed round trip')
    return snapshots
