"""Private elastic-seed-owned conservative current-rest factor capture.
...
from . import _ge_beam3_elastic_seed_continuation as control
...
POLICY = 'GE_BEAM3_ELASTIC_SEED_OWNED_CURRENT_REST_FACTOR_CHAIN_V1'
...
class ElasticSeedPencil(Pencil):
    seed_sha256: str = ''
...
def prepare(model, program, seed, checkpoint, section_inertias, *, expected_seed_sha256,
    ...
    context = control.Context(model, program, seed, expected_seed_sha256=expected_seed_sha256)
    ...
    expanded = left@right
    target = context.seed_target if state.completed_targets == 0 else program.targets[state.completed_targets-1]
    ...
    packet = ElasticSeedPencil(**body, seed_sha256=expected_seed_sha256, identity=sha(dict(policy=POLICY,**body)), policy=POLICY)
    identity = sha(packet)
