...
def _shell_corner_metrics(coords: np.ndarray) -> Dict[str, float]:
    ...
    if coordinates.ndim != 2 or coordinates.shape[1] != 3:
        raise ValueError(
            "shell coordinates must have shape (3|4|6|8, 3)"
        )
    ...
    if node_count in (3, 6):
        ...
        raise ValueError(
            f"shell quality supports 3, 4, 6 or 8 nodes, got {node_count}"
        )
    if not np.all(np.isfinite(corner)):
        raise ValueError("shell corner coordinates must be finite")

    edges = [
        corner[(index + 1) % len(corner)] - corner[index]
