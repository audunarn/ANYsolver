...
import numpy as np
from ._ge_beam3_native_generalized_element import NativeGeneralizedStaticElement,FORMULATION,SCHEMA
...
def recover_native_fields(element,mesh,state,*,expected_committed_total_u=None):
    if type(element) is not NativeGeneralizedStaticElement: raise ValueError('exact native generalized resultant recovery element')
    ...
    return dict(formulation_id=FORMULATION,state_schema=SCHEMA,load_pattern_sha256=state['load_pattern'].signature,state_sha256=state['state_sha256'],element_id=element.element_id,
        ...
        fibre_stress_status='RESULTANT_LEVEL_ONLY_NO_FIBRE_STRESSES_SUPPLIED',stations=tuple(rows),production_qualified=False)
