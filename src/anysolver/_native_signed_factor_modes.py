...
def _reduce(factor, geometric, mass, free_dofs, algebraic_dofs, checkpoint):
...
class SignedFactorModes:
    ...
    policy: str = 'PRIVATE_SIGNED_MATERIAL_GEOMETRIC_FACTOR_MODES_V1'
...
def solve_signed_factor_modes(factor, geometric, mass, free_dofs, algebraic_dofs, *,
    ...
    f, g, m = (_owned(x) for x in (factor, geometric, mass))
    ...
    d, correction, mapping = _reduce(f, g, m, free_dofs, algebraic_dofs, checkpoint)
