...
from ._ge_beam3_fibre_distributed_couples import POLICY
...
_ACTIVE=ContextVar('ge_beam3_native_distributed_scope',default=None)
...
def assemble_distributed_trial(model,displacements,store,pattern,*,tangent=True):
    ...
    if not model.mesh.elements or any(type(e) is not NativeDistributedFibreStaticElement for e in model.mesh.elements.values()):
