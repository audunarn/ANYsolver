"""Private frozen M_S3 reference problem; no finite mixed or restart admission.

This separate development slice does not alter the preceding beam interface,
accepted S3 V2D operators, or public graph/element ownership.
"""
...
POLICY = "G3B_M_S3_REFERENCE_LINEAR_TRANSLATIONS_ONLY_DEVELOPMENT_V1"


class S3TranslationReferenceProblem:
    """One reference-native beam and one explicit accepted planar S3 V2D.
    ...
    def __init__(self, native, shell, material, *, nodes, fixed_nodes, tie,
        ...
        if (type(self) is not S3TranslationReferenceProblem or policy != POLICY or
                type(adapter_allowlist) is not tuple or adapter_allowlist or rotation_targets is not None):
            raise ValueError("M_S3 translation-only reference policy required")
        if type(native) is not ElasticElement or type(shell) is not NativeParityE4PLS3V2DShellElement or type(material) is not Material:
            raise ValueError("exact native/accepted S3 V2D/isotropic material required")
        ...
        if (type(shell.element_id) is not int or shell.element_id <= 0 or native.element_id == shell.element_id or
                len(shell.node_ids) != 3 or any(type(n) is not int or n <= 0 for n in shell.node_ids) or
                len(set(shell.node_ids)) != 3 or set(native.node_ids) & set(shell.node_ids)):
            raise ValueError("disjoint exact element/node identities required")
        if (type(nodes) is not dict or len(nodes) != 6 or
                any(type(n) is not int or n <= 0 for n in nodes) or
                set(nodes) != set(native.node_ids) | set(shell.node_ids)):
            raise ValueError("exact six-node M_S3 inventory required")
        ...
        if (shell.shell_section is not None or shell.material_direction is not None or shell.material_angle_deg != 0 or
                shell.reduced_integration or shell.reference_surface_offset != 0 or
                shell.drilling_stabilization != 0 or shell.hourglass_stabilization != 0 or
            ...
            raise ValueError("frozen homogeneous reference-elastic M_S3 inputs required")
        if (shell.__dict__.get('_stiffness_matrix') is not None or shell.__dict__.get('_nl_cache') is not None or
                shell.__dict__.get('_qualified_direct_state_token') is not None or
                shell.__dict__.get('_qualified_direct_state_tokens')):
            raise ValueError("fresh reference S3 V2D without prior caches/state required")
        ids = tuple(sorted(nodes)); positions = owned([nodes[n] for n in ids], (6, 3))
        ...
        # This first slice is the frozen planar triangle, not a geometry envelope.
        if not np.array_equal(sx, [[0.,0.,0.], [1.,0.,0.], [1.,1.,0.]]):
            raise ValueError("frozen M_S3 ordered triangle required")
        if (type(fixed_nodes) is not tuple or len(fixed_nodes) != 3 or
            ...
            raise ValueError("M_S3 native root and opposite shell edge supports required")
        ...
        self.ids, self.positions, self.size, self.normal = ids, positions, 36, normal
        self.fixed_nodes, self._tie = tuple(sorted(fixed_nodes)), canonical(expected)
        self._lock = threading.Lock()
        self.model = FEModel('G3b M_S3 reference development')
    ...
    def _guard(self):
        if (type(self) is not S3TranslationReferenceProblem or type(self.native) is not ElasticElement or
                type(self.shell) is not NativeParityE4PLS3V2DShellElement or type(self.material) is not Material or
                self.native._mesh is not None or self.native._owner_guard is not None):
            raise ValueError('M_S3 reference-only ownership changed')
        ...
        if (self.shell.__dict__.get('_nl_cache') is not None or
            ...
            raise ValueError('M_S3 reference cache ownership/epoch changed or nonlinear cache present')
        if (self.model.constraint_equations or self.model.boundary_conditions or self.model.load_cases or
                ...
                self.model.mesh.element_activity is not None or self.size != 36 or
                self.model.mesh.dof_manager.total_dofs != 36):
            raise ValueError('unsupported M_S3 graph feature')
        if (tuple(self.model.mesh.nodes) != self.ids or len(self.model.mesh.elements) != 2 or
            ...
            raise ValueError('frozen M_S3 graph/operator changed')
        self.native.operator.guard(); self.shell._validate_configuration()

    def solve(self, nodal, *, mode='REFERENCE_LINEAR', rotation_targets=None, cancel=lambda:False):
        if type(self) is not S3TranslationReferenceProblem or mode != 'REFERENCE_LINEAR' or rotation_targets is not None:
            raise ValueError('finite mixed/rotational coupling remains unsupported')
        if not self._lock.acquire(blocking=False): raise RuntimeError('M_S3 problem already in use')
        try:
            self._guard(); f = owned(nodal,(36,))
            if cancel(): raise InterruptedError('M_S3 cancelled')
            self._guard()
            u = self.T @ solve(self.reduced,self.T.T @ f,assume_a='pos')
            balance = self.K @ u-f
            mu = solve(self.J @ self.J.T,-self.J @ balance,assume_a='pos')
            invariant(balance+self.J.T @ mu,np.zeros(36)); invariant(self.J @ u,np.zeros(21))
            ...
            if (recovered['numerical_fields_excluded'] is not True or
                    recovered['formulation_id'] != self.shell.formulation_id or
                    not np.array_equal(recovered['frame'][:,2],self.normal)):
                raise ValueError('M_S3 physical recovery authority mismatch')
            ...
            progress('recovery')
            if cancel(): raise InterruptedError('M_S3 cancelled before return')
    ...
    def checkpoint(self):
        raise ValueError('M_S3 development has no restart/state authority; full G3b owner is later')
