"""Private native distributed-couple static element with issued trial ownership.

The preserved physical potential and the frozen general distributed-couple
...
from ._ge_beam3_fibre_distributed_couples import solve_distributed_static as solve_static,DistributedStaticTrial,cell_couple_load,spatial_jacobian,chart_pullback,POLICY as STATIC_POLICY
...
from ._ge_beam3_native_distributed_loading import DistributedPattern,current_pattern
from ._ge_beam3_fibre_line_work import evaluate as line_work

FORMULATION='CANDIDATE_GE_BEAM3_NATIVE_PHYSICAL_FIBRE_DISTRIBUTED_STATIC_V1'
SCHEMA='GE_BEAM3_NATIVE_PHYSICAL_FIBRE_DISTRIBUTED_STATIC_STATE_V1'
...
class NativeDistributedFibreStaticElement(Element):
    num_nodes=3
    dofs_per_node=6
    ...
    def _check(self,mesh):
        if type(self) is not NativeDistributedFibreStaticElement or mesh.elements.get(self.element_id) is not self:
        ...
        if any(type(e) is not NativeDistributedFibreStaticElement for e in mesh.elements.values()): raise ValueError('private standalone native fibre adapter only')
        self.operator.guard()
    ...
    def _validate(self,mesh,state,expected=None):
        ...
        if type(pattern) is not DistributedPattern:raise ValueError('exact native distributed state pattern')
        ...
        if type(response) is not DistributedStaticTrial: raise ValueError('exact native fibre static response')
        ...
        expected_response=DistributedStaticTrial(response.rotations,response.resultants,net[:18],condensed,lift,
            net,residual,h,jac,applied,replay.history,float(potential),error,response.iterations,input_id)
        if not np.isfinite(error) or error>1e-11 or canonical(expected_response)!=canonical(response): raise ValueError('native fibre static/history replay mismatch')
        return state
    ...
    def create_native_material_validator(self,mesh):
        ...
        def validate(state,*,previous_state,native_view,displacement,phase):
            if phase=="trial":
                issued=self._issued
                if issued is None or state.get("state_sha256")!=issued[1]:raise ValueError("native distributed candidate was not issued by its load scope")
                binding=issued[0].store._native_element_bindings.get(self.element_id)
                if binding is None or binding.material_validator is not validator:raise ValueError("foreign native distributed validator")
