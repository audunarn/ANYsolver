...
import anysolver._ge_beam3_g3c_b2_physical_adapter as physical_b2
import anysolver._ge_beam3_g3c_affine_q4_recovery as physical_q4
import anysolver._ge_beam3_g3c_affine_q4_registry as q4_registry
...
import anysolver._ge_beam3_g3c_physical_authority as definition_module
from anysolver._ge_beam3_g3c_physical_authority import expand
from anysolver._ge_beam3_g3c_physical_authority import command
from anysolver._ge_beam3_g3c_physical_authority import CONTRACT_SHA
from anysolver._ge_beam3_g3c_physical_authority import U
from anysolver._ge_beam3_g3c_physical_authority import SHIFT
...
SCHEMA='GE_BEAM3_G3C_PHYSICAL_MIXED_ELASTIC_STATE_V1'
...
def dispatch():
    modules=(native_module,operator_module,beam_module,shell_module,joint_module,
             state_module,legacy_module,rotation_module,physical_b2,physical_q4,q4_registry,
...
def physical_equal(a,b,label):
    a,b=np.asarray(a),np.asarray(b)
    scale=np.linalg.norm(a)+np.linalg.norm(b)
    error=np.linalg.norm(a-b)
    if not np.isfinite(a).all() or not np.isfinite(b).all() or (error>1e-11*scale if scale else error!=0):
        raise ValueError(label)

def recovery_witness(family,trial):
    """Bind actual detached recovery, never a claimed recovery-complete flag."""
    if family=='B2':
        force=np.einsum('i,ijk,ij->k',trial.weights,trial.chart_strain_differential,trial.resultants)
        physical_equal(force,trial.chart_force,'B2 physical station work')
        energy=.5*float(np.einsum('i,ij,ij->',trial.weights,trial.strains,trial.resultants))
        physical_equal(energy,trial.energy,'B2 physical energy')
        witness=(trial.strains,trial.resultants,trial.chart_strain_differential,
                 trial.weights,trial.reference_frame,trial.current_frame,trial.energy)
    elif family=='Q4':
        force=sum(st.weight*st.chart_first.T@st.resultant for st in trial.stations)
        hessian=sum(st.weight*(st.chart_first.T@st.constitutive@st.chart_first+
        ...
        physical_equal(trial.internal_inverse64@trial.internal_block64,np.eye(64),'Q4 left inverse')
        ...
        witness=trial.channels
        if not witness: raise ValueError('missing S3 recovery channels')
    else: raise ValueError('foreign recovery family')
    return physical_q4._fingerprint(witness)
...
class MixedGraphOwner:
    ...
    def __init__(self,fixture_id='J_B2_PAIR',variant='BASE',common_motion='NONE',*,
                 policy=POLICY,operator_graph_id='GE_BEAM3_G3C_PHYSICAL_OPERATOR_GRAPH_V1',
                 state_schema=SCHEMA):
        if type(self) is not MixedGraphOwner: raise ValueError('exact owner required')
        if (type(policy) is not str or policy!=POLICY or type(state_schema) is not str
            or state_schema!=SCHEMA or type(operator_graph_id) is not str
            or operator_graph_id!='GE_BEAM3_G3C_PHYSICAL_OPERATOR_GRAPH_V1'):
            raise ValueError('historical or foreign physical owner identity')
        ...
        for name,value in dict(_definition=canonical(definition),_expanded=canonical(expanded),
            _programs=canonical(programs),_runtime=sha(dict(source=runtime_digest(),environment=ENVIRONMENT,physical=definition_module.runtime_identity())),
        ...
        cache=tuple((e['id'],physical_q4.AffineQ4PhysicalRecovery(fixture_id+'::'+variant))
            for e in expanded['graph']['elements'] if e['family']=='Q4')
        object.__setattr__(self,'_q4_cache',cache)
        object.__setattr__(self,'_owned_q4_cache',cache)
        self._check_q4_registry()
    ...
    def _guard(self,origin=None,*,full=False,lock=None,nonce=None):
        try:
            current=dispatch()
            ...
            if (type(self) is not MixedGraphOwner or self._poisoned or
                ...
                (nonce is not None and self._active is not nonce) or
            ...
            if full:
                ...
                if (canonical(definition)!=self._definition or canonical(expanded)!=self._expanded or
                    canonical(programs)!=self._programs or sha(dict(source=runtime_digest(),environment=ENVIRONMENT,physical=definition_module.runtime_identity()))!=self._runtime):
                    raise ValueError('frozen source or definition changed')
            # Registry/authority/dispatch observations are fallible. Validate
            # capabilities AGAIN after them; an observer cannot replace the
            # nonce or release/replace the acquired lock in the last window.
            if dispatch()!=self._dispatch:
                raise ValueError('runtime dispatch changed during guard')
            if (self._poisoned or self._lock is not self._owned_lock
                or self._q4_cache is not self._owned_q4_cache
                or (lock is not None and (self._lock is not lock or not lock.locked()))
                or (nonce is not None and self._active is not nonce)
                or (origin is not None and self._published is not origin)):
                raise ValueError('captured capability changed during guard')
    ...
    def _check_q4_registry(self):
        data=json.loads(self._expanded); definition=json.loads(self._definition)
        points=dict(data['graph']['nodes'])
        expected=[e for e in data['graph']['elements'] if e['family']=='Q4']
        if tuple(e['id'] for e in expected)!=tuple(i for i,_ in self._q4_cache):
            raise ValueError('foreign Q4 cache membership')
        direction=[1.,0.,0.] if definition['variant']!='PROPER_GLOBAL_TRANSFORM' else [0.,1.,0.]
        for row,(eid,local) in zip(expected,self._q4_cache):
            desc=local.descriptor()
            if (type(local) is not physical_q4.AffineQ4PhysicalRecovery
                or desc['construction_id']!=definition['fixture_id']+'::'+definition['variant']
                or tuple(desc['node_ids'])!=tuple(row['nodes'])
                or desc['coordinates']!=[points[n] for n in row['nodes']]
                or desc['normal']!=data['definitions']['shell']['reference_normal']
                or desc['material_direction']!=direction):
                raise ValueError('graph and registered Q4 definition disagree')
    ...
    def _adapter_rows(self,total,qa,q,*,epoch,previous,check):
        ...
        for row in g['elements']:
            ...
            if row['family'] in ('B2','B3'):
                ...
                local=(physical_b2.PhysicalB2Adapter(tuple(row['nodes']),x,anchors[row['id']],d['E'],d['nu'],section)
                    if row['family']=='B2' else
                    beam_module.LocalBeam('B3',tuple(row['nodes']),x,anchors[row['id']],d['E'],d['nu'],section))
                trial=local.evaluate(u,owned(qa[indices])); diagnostic_hash=None
            elif row['family']=='Q4':
                self._check_q4_registry()
                local=dict(self._q4_cache)[row['id']]
                trial=local.evaluate(u,owned(qa[indices]))
                diagnostic_hash=sha256(trial.candidate).hexdigest()
            else:
                direction=np.array([1.,0.,0.])
                if json.loads(self._definition)['variant']=='PROPER_GLOBAL_TRANSFORM': direction=np.asarray(U)@direction
                local=shell_module.LocalShell('S3-V2D',tuple(row['nodes']),x,
            ...
            envelopes.append(dict(**definition_module.envelope_identities(row['family']),
                recovery_witness_sha256=recovery_witness(row['family'],trial),
                element_id=row['id'],policy=data['local_policies'][row['family']],
    ...
    def _run(self,cmd,total,mu,*,hook,solve_graph):
        ...
        try:
            ...
            def check(stage):
                ...
                self._guard(origin,lock=lock,nonce=nonce)
                if hook is not None:
                    hook(stage)
                    self._guard(origin,full=True,lock=lock,nonce=nonce)
            ...
            for iteration in range(25):
                ...
                if error<=1e-11:
                    ...
                    check('before_publish'); self._guard(origin,full=True,lock=lock,nonce=nonce)
            ...
            object.__setattr__(self,'_active',None)
            # A hostile observer may already have released this primitive.
            # Never release a replacement read back from the owner.
