"""Private physical-fibre static adapter using actual native state tokens.
...
FORMULATION='CANDIDATE_GE_BEAM3_NATIVE_PHYSICAL_FIBRE_STATIC_V1'
SCHEMA='GE_BEAM3_NATIVE_PHYSICAL_FIBRE_STATIC_STATE_V1'
...
class NativeFibreStaticElement(Element):
    num_nodes=3
    dofs_per_node=6
    ...
    def __init__(self,element_id,node_ids,reference,section,*,order=4):
        ...
        self._mapping=None; self._mesh=None; self._validator=None

    def _check(self,mesh):
        if type(self) is not NativeFibreStaticElement or mesh.elements.get(self.element_id) is not self:
        ...
        if any(type(e) is not NativeFibreStaticElement for e in mesh.elements.values()): raise ValueError('private standalone native fibre adapter only')
        self.operator.guard()
    ...
    def _state(self,epoch,total,operators,origin,seed_u,seed_p,response,previous):
        ...
        return seal(dict(schema=SCHEMA,element_identity=self.identity,epoch=epoch,previous_state_sha256=previous,
            ...
            origins=origin,seed_rotations=_owned(seed_u),seed_resultants=_owned(seed_p),response=response))
    ...
    def _validate(self,mesh,state,expected=None):
        ...
        if epoch==0 and (state['previous_state_sha256'] is not None or np.any(total) or not np.array_equal(operators,np.tile(np.eye(3),(3,1,1))) or canonical(state['origins'])!=canonical(self.operator.cell.virgin())):
        ...
        input_id=sha(dict(policy=STATIC_POLICY,operator=self.operator.identity,positions=positions,position_low=low,
            nodal_frames=q,origin=state['origins'],line_force=np.zeros(3),initial_rotations=seed_u,initial_resultants=seed_p))
        ...
        error=float(np.linalg.norm(replay.residual[18:]/np.r_[np.full(12,length),np.ones(12)]))
        expected_response=StaticTrial(response.rotations,response.resultants,replay.residual[:18],condensed,
            replay.residual,h,replay.history,replay.potential,error,response.iterations,input_id)
    ...
    def init_model_bound_nonlinear_state(self,mesh,material,num_layers):
        ...
        return self._state(0,total,operators,origin,seed_u,seed_p,response,None)
    ...
    def create_native_material_validator(self,mesh):
        self._check(mesh)
        if self._validator is not None: return self._validator
        ...
        self._validator=NativeMaterialValidator(validate)
    ...
    def compute_nonlinear_response(self,mesh,material,displacement,state=None,num_layers=1,tangent=True,*,native_rotation_trial=None,native_material_context=None):
        ...
        self._live(context,view,state); previous=self._validate(mesh,state); self._pose(previous,view,committed=True)
        ...
        response=solve_static(self.operator,positions,low,view.trial_rotation_matrices@self.operator.reference.nodal_triads,
            origin=origin,initial_rotations=seed.rotations,initial_resultants=seed.resultants,spatial_line_force=np.zeros(3),check=lambda: self._live(context,view,state,check_state=False))
        force,matrix,_=pullback(response.residual,response.hessian,view.rotation_coordinate_increment)
        self._live(context,view,state)
        candidate=self._state(previous['epoch']+1,total,view.trial_rotation_matrices,origin,seed.rotations,seed.resultants,response,previous['state_sha256'])
