"""G3 native-only graph constraints. G2 equations, separately bounded capture.

This is not a cross-family pose adapter. Only NativeGraphAnalysis admits it.
Accepted G2 code and bounds are unchanged.
"""
...
SCHEMA = "GE_BEAM3_G3_NATIVE_CONSTRAINTS_V1"
...
def compile_affine(size, rows):
    ...
    if type(size) is not int or not 1 <= size <= 192 or type(rows) not in (tuple, list) or len(rows) > size:
...
class GraphConstraintSet:
    _raw: bytes

    def __init__(self, node_ids, positions, frames, *, affine=(), orientations=(), poses=()):
        if (type(node_ids) is not tuple or not 3 <= len(node_ids) <= 32 or
        ...
        if tuple(sorted(node_ids)) != node_ids:
            raise ValueError("G3 requires sorted node/DOF identities")
