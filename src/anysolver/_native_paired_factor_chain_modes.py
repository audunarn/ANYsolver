"""Private paired-vector successor of the preserved relative factor kernel.
...
import json
from ._paired_modal_arithmetic import orthogonal_coordinates, paired_expansion, paired_actions, paired_map
...
def brackets(h,mass,bounds,count,absolute,relative,checkpoint):
    ...
    for index in range(count):
        ...
        for _ in range(192):
            checkpoint()
            mid=lo+(hi-lo)/2
            width=absolute+relative*abs(mid)
        ...
        else:raise ValueError('relative signed search iteration limit')
        width=absolute+relative*abs(lo+(hi-lo)/2)
        if hi-lo>width:raise ValueError('relative signed search width not achieved')
        results.append((lo,hi));widths.append(width)
    return np.array(results),np.array(widths)


@dataclass(frozen=True)
class PairedFactorChainModes:
    eigenvalues: np.ndarray
    high_modes: np.ndarray
    ...
    policy: str='PRIVATE_COMPLETE_MAP_PAIRED_FACTOR_CHAIN_SIGNED_MODES_V1'
    bracket_absolute_width: float=1e-10
    bracket_relative_width: float=1e-12
    ...
    @property
    def full_modes(self):
        raise AttributeError('paired modes require explicit high/low action; no validated high-only array')


def _fingerprint(result):
    payload={f.name:getattr(result,f.name) for f in fields(result) if f.name!='fingerprint'}
    return sha256(json.dumps(payload,sort_keys=True,separators=(',',':'),allow_nan=False,
        default=lambda a:a.tolist()).encode()).hexdigest()


def validate_modes(result):
    if type(result) is not PairedFactorChainModes or _fingerprint(result)!=result.fingerprint:
        raise ValueError('paired modal result identity changed')
    if (result.high_modes.shape!=result.low_modes.shape or result.high_modes.shape[1]!=len(result.eigenvalues)
            or any(a.flags.writeable for a in (result.high_modes,result.low_modes,result.eigenvalues,result.numerical_brackets))):
        raise ValueError('owned matching immutable paired modal arrays required')
    return result


def apply_mode_map(result,operator,checkpoint=lambda:None):
    validate_modes(result)
    output=paired_map(operator,result.high_modes,result.low_modes,checkpoint)
    validate_modes(result)
...
def solve_paired_factor_chain_modes(left_factor,right_factor,geometric,kinetic,free,algebraic,*,bounds,num_modes=6,root_width=1e-10,relative_width=1e-12,cancellation_token=None):
    ...
    if (type(bounds) is not tuple or len(bounds)!=2 or any(type(v) not in (float,int) or not isfinite(v) for v in bounds)
            ...
            or type(relative_width) is not float or not isfinite(relative_width) or not 0<relative_width<=1e-10):
    ...
    intervals,widths=brackets(h,mass,bounds,num_modes,root_width,relative_width,checkpoint)
    coordinates=[]; values=[]; index=0
    while index<num_modes:
        end=index+1
        while end<num_modes and intervals[end,0]-intervals[end-1,1]<=8*max(widths[end],widths[end-1]): end+=1
        ...
        cluster_width=float(np.max(widths[index:end]))
        shifted_matrix,scale=shifted(h,mass,float(intervals[index,0]-2*cluster_width))
        ...
        if np.any(rv<intervals[index:end,0]-widths[index:end]) or np.any(rv>intervals[index:end,1]+widths[index:end]):
            raise ValueError('reassembled Ritz roots leave numerical brackets')
        coordinates.append(actual); values.extend(rv); index=end
    c=orthogonal_coordinates(np.column_stack(coordinates),mass,checkpoint); values=np.array(values)
    ...
    high,low=paired_expansion(mapping,c,checkpoint)
    original_h,original_m,full_action,material_full,geometric_full,inertia_full=paired_actions(
        left,right,g,b,high,low,values,checkpoint)
    if np.linalg.norm(original_m-np.eye(num_modes))>1e-11:
        raise ValueError('physical modal mass normalization')
    # Audit the ORIGINAL signed form using both physical vector halves.
    ...
    for j in range(num_modes):
        if high[int(np.argmax(np.abs(high[:,j]))),j]<0.:
            high[:,j]*=-1.;low[:,j]*=-1.
    # Same normwise full backward threshold as the preserved kernel, now
    # with exact original-chain actions rather than an expanded Gram product.
    scale=max(1.,(np.linalg.norm(f)**2+np.linalg.norm(g))*np.linalg.norm(high),
        np.linalg.norm(dense_mass)*np.linalg.norm(high*values))
    if not isfinite(scale): raise ValueError('nonfinite full backward scale')
    backward=float(np.linalg.norm(full_action[list(free)])/scale)
    if not isfinite(backward) or backward>1e-11: raise ValueError('full backward residual')
    checkpoint('output')
    result=PairedFactorChainModes(_owned(values),_owned(high),_owned(low),_owned(intervals),
        error,backward,original_error,bracket_absolute_width=root_width,bracket_relative_width=relative_width)
    result=replace(result,fingerprint=_fingerprint(result))
    return validate_modes(result)
