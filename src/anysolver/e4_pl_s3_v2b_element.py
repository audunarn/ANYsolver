"""Strict flat-linear E4-PL S3 V2B production candidate.
...
globally coplanar qualified-Q4/V2B mesh with one positively aligned physical
...
IMPLEMENTATION_ID = "E4_PL_S3_V2B_MIN3_RELAXED_UHM_CST_PL_HAMMER3_V1"
FORMULATION_SCHEMA = "anysolver.e4-pl-s3-v2b-strict-flat-linear-element-v1"
SOURCE_CONTRACT_SCHEMA = "anysolver.e4-pl-s3-v5b-relaxation-source-selection-v1"
...
QUADRATURE_AUTHORITY_ID = "S3_V2B_MIN3_HAMMER3_DEGREE2_EXACT_V1"
...
_MIXED_SCOPE_CACHE_NAME = "_strict_flat_v2b_mixed_scope_cache_v1"
_COMPONENT_CACHE_SCHEMA = "strict-flat-s3-v2b-exact-component-cache-v1"
_COMPONENT_CACHE_ENTRY_SCHEMA = (
    "strict-flat-s3-v2b-exact-component-cache-entry-v1"
)
...
def _make_exact_component_compute_wrapper() -> Any:
    ...
    compute_stiffness_components.__qualname__ = (
        "StrictFlatLinearE4PLS3V2BShellElement.compute_stiffness_components"
    )
...
class StrictFlatLinearE4PLS3V2BShellElement(
    ...
    def __init__(
        ...
        *,
        ...
        if len(owned_node_ids) != 3:
            raise ValueError(
                "StrictFlatLinearE4PLS3V2BShellElement requires exactly three nodes"
            )
    ...
    def _validate_model_scope(self, mesh: FEMesh) -> None:
        """Admit only one exact, coplanar qualified-Q4/V2B model boundary.
        ...
        if elements.get(self.element_id) is not self:
            raise StrictFlatLinearCapabilityError(
                ...
                "evaluated V2B instance exactly"
            )
        ...
        for key, element in registered_items:
            if type(key) is not int or type(element) not in allowed_types:
                raise StrictFlatLinearCapabilityError(
                    ...
                    "qualified Q4 and V2B shell elements"
                )
    ...
    def compute_dead_transverse_pressure_load(
        ...
        if isinstance(pressure, (int, float, np.integer, np.floating)):
            ...
            raise StrictFlatLinearCapabilityError(
                "strict-flat S3 V2B source authority admits uniform pressure only"
            )
        local = np.asarray(geometry["local_coordinates"], dtype=np.float64)
        a = np.empty(3, dtype=np.float64)
        b = np.empty(3, dtype=np.float64)
    ...
    def __copy__(self) -> "StrictFlatLinearE4PLS3V2BShellElement":
        self._unsupported("restart")
...
def _sealed_inherited_operator(name: str) -> Any:
    def sealed(self: StrictFlatLinearE4PLS3V2BShellElement, *args: Any, **kwargs: Any) -> Any:
        del args, kwargs
        self._unsupported(f"legacy_inherited_operator:{name}")

    sealed.__name__ = name
    sealed.__qualname__ = f"StrictFlatLinearE4PLS3V2BShellElement.{name}"
    return sealed
...
_initialize_final_class_authority(StrictFlatLinearE4PLS3V2BShellElement)
