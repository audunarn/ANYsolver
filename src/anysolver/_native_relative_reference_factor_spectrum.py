"""Private relative-accuracy reference-energy spectrum successor.

Derived from preserved V1; its guards and mechanical factor semantics remain.
Only the terminal SVD driver changes. V1 and its evidence are unmodified.
...
POLICY = 'PRIVATE_REFERENCE_ENERGY_KINETIC_QR_JACOBI_V2'


@dataclass(frozen=True)
class RelativeReferenceFactorSpectrum:
...
def solve_relative_reference_factor_spectrum(elastic_factor, kinetic_factor, free_dofs, algebraic_dofs, *,
    ...
    def checkpoint(stage):
        cancellation_safe_point(cancellation_token, 'relative_reference_factor_spectrum.'+stage)
    checkpoint('start')
    ...
    with np.errstate(over='raise', invalid='raise', under='raise'):
        try:
            ...
            raise ValueError('unrepresentable squared reference frequency') from exc
    ...
    checkpoint('output')
    return RelativeReferenceFactorSpectrum(_owned(values), _owned(modes), _owned(mapping), error)
