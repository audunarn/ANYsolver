...
from ._ge_beam3_native_arc import capture,arc_row,replay_direction
...
def encode_checkpoint(model,program,chain,records):
    ...
    if len(observed)>MAX_BYTES:raise ValueError('native arc checkpoint byte limit')
    inner=encode_states(model,chain);old_parameter=0.;fixed=np.setdiff1d(np.arange(n),free)
    ...
    body=dict(schema=SCHEMA,model_sha256=identity,program=program.descriptor(),program_sha256=sha(program.descriptor()),
        metric=metric,records=records,accepted_chain=_load(inner.decode('ascii')),production_qualified=False)
    raw=canonical({**body,'checkpoint_sha256':sha(body)})
    if len(raw)>MAX_BYTES:raise ValueError('native arc checkpoint byte limit')
    return raw
...
def decode_checkpoint(model,program,raw,*,expected_sha256):
    ...
    _keys(value,('schema','model_sha256','program','program_sha256','metric','records','accepted_chain','production_qualified','checkpoint_sha256'))
