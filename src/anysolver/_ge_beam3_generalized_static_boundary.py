...
POLICY = 'GE_BEAM3_GENERALIZED_REFERENCE_SPATIAL_DISTRIBUTED_COUPLE_STATIC_V1'


@dataclass(frozen=True)
class GeneralizedStaticTrial:
...
def cell_couple_load(operator, spatial_density):
    if type(operator) is not RetainedGeneralizedOperator:
        raise ValueError('exact generalized retained operator required')
    operator.guard()
...
def solve_distributed_static(operator, positions, position_low, nodal_frames, *, origin,
    ...
    if type(operator) is not RetainedGeneralizedOperator or type(origin) is not GeneralizedCellHistory:
    ...
    for iteration in range(max_iterations+1):
        ...
        if error <= 1e-11:
            ...
            safe()
            return GeneralizedStaticTrial(_owned(u), _owned(p), _owned(net[:18]), _owned(condensed),
