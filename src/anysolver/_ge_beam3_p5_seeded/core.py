...
from anysolver._ge_beam3_p5_centered.positions import POLICY, from_total, validate_total_pair
from .state import LOAD_POLICY, parameter_value

from anysolver._ge_beam3_authoritative_cell_seed import cell_initial_rotations, POLICY as SEED_POLICY

SCHEMA='GE_BEAM3_P5_LOAD_STATION_STATE_V5'


FORMULATION='CANDIDATE_GE_BEAM3_P5_LOAD_CORE_V5'
...
class StationaryCore(Element):
    ...
    def __init__(self, element_id, node_ids, reference, section, *, line_force, order=8):
        ...
        self.line_force=_array(line_force,(3,), 'unit reference-arclength line force')
    ...
    def _identity(self):
        return dict(formulation=FORMULATION, schema=SCHEMA, coordinate_policy=POLICY, initial_rotation_policy=SEED_POLICY, element=self.element_id, nodes=self.node_ids,
            reference_evaluation=self.reference.evaluation_id, reference_fingerprint=self.reference.fingerprint(),
            load_policy=LOAD_POLICY, line_force=self.line_force,
    ...
    def _solve(self, positions, operators, origins, *, total, parameter):
        ...
        model=CompensatedStationaryBeam(self.reference, self.section, order=self.order,
            position_low=low, origins=origins, line_force=parameter_value(parameter)*self.line_force)
    ...
    def _state(self, epoch, total, positions, operators, origins, response, *, parameter):
        positions, low = from_total(self.reference.coordinates, total, positions)
        return seal(dict(schema=SCHEMA, model_sha256=self.model_sha256, epoch=epoch,
            ...
            load_parameter=parameter_value(parameter),
    ...
    def init_model_bound_nonlinear_state(self, mesh, material=None, num_layers=1):
        ...
        response=self._solve(self.reference.coordinates, operators, origins, total=np.zeros(18), parameter=0.)
        return self._state(0, np.zeros(18), self.reference.coordinates, operators, origins, response, parameter=0.)
    ...
    def validate_model_bound_nonlinear_state(self, mesh, material, state, num_layers,
        ...
        keys={'schema', 'model_sha256', 'epoch', 'committed_total_u', 'committed_positions',
              'coordinate_policy', 'committed_position_low', 'load_parameter',
        ...
        parameter=parameter_value(state['load_parameter'])
        if state['epoch']==0:
            if (parameter!=0. or np.any(u) or not np.array_equal(positions, self.reference.coordinates)
        ...
        replay=self._solve(positions, operators, state['origins'], total=u, parameter=parameter)
    ...
    def _load_parameter_derivative(self, state):
        # Caller has replay-validated this native state and bound the live trial.
        from .work import load_work
        response=state['response']; p=parameter_value(state['load_parameter'])
        model=CompensatedStationaryBeam(self.reference,self.section,order=self.order,
            position_low=state['committed_position_low'],origins=state['origins'],line_force=p*self.line_force)
        full=model.evaluate(state['committed_positions'],
            state['committed_nodal_rotation_matrices']@self.reference.nodal_triads,
            response.local_rotations,response.moments)
        unit=load_work(self.reference,state['committed_positions'],state['committed_position_low'],
            response.local_rotations,self.line_force,order=self.order)
        h=full.hessian; dz=np.linalg.solve(h[18:,18:],unit.force[18:])
        gradient=-unit.force[:18]+h[:18,18:]@dz
        if not np.isfinite(gradient).all(): raise NativeMaterialError('nonfinite load-parameter derivative')
        return gradient, -unit.work
